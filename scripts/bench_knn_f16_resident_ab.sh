...
#
# 計測順序（軽い規模点から先に完走させる。要件 A〔前後比較〕を要件 B〔常駐
# メモリ〕より優先する——`docs/design/hnsw-f16-resident.md` の受け入れ条件 1
# が本 Issue の主目的のため）:
#   1. 索引単体メモリモード（5 SQL 到達可能点 × {hnsw, hnsw_f16} × 2 回）
#   2. hot-only レイテンシ（25k dim128 → 100k dim128 → 500k dim128 →
...
read -r -a AB_MEMORY_POINTS_ARR <<<"${AB_MEMORY_POINTS:-${DEFAULT_POINTS} 20:768}"
