...
# 展開・`cargo build --release -p fandhe-vector-db-wire-server` で個別ビルドし、
...
prepare_arm() {
  ...
  local bin
  bin="$(cd "${wt}" && CARGO_TARGET_DIR="${target}" cargo build --release -p fandhe-vector-db-wire-server --message-format=json | locate_artifact)"
