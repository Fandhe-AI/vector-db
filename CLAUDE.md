...
- ステータス: workspace 雛形構築済み（TASK-66）。wire プロトコル層は実装済み（TASK-67）。接続資源保護（読み取りタイムアウト・共有接続数リミッター、TASK-69）実装済み。他は未実装（タスクは spec リポの `05-tasks.md`（TASK-66〜154）、マイルストーンは `06-roadmap.md`（MS-1〜6）参照）
