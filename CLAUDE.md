...
- ステータス: workspace 雛形構築済み（TASK-66）。wire プロトコル層（TCP リスナー・cleartext password 認証・Argon2id/BLAKE2b 自作実装）は実装済み（TASK-67）。他は未実装（タスクは spec リポの `05-tasks.md`（TASK-66〜154）、マイルストーンは `06-roadmap.md`（MS-1〜6）参照）
