...
| 種類 | 説明 |
|------|------|
| Branch rulesets | ブランチに対するルール（`fnmatch` 構文でターゲット指定） |
| Tag rulesets | タグに対するルール（`fnmatch` 構文でターゲット指定） |
| Push rulesets | プッシュに対するルール（ファイルパス・長さ・拡張子・サイズで制限）。リポジトリおよびそのフォークネットワーク全体に適用される。プライベート・インターナルリポジトリのみ利用可能 |
