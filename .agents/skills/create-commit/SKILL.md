...
`--no-verify` は使わない。pre-commit フックはシークレット検査や lint の最後の防衛線なので、失敗したら原因を修正して再コミットする。
...
| シークレットチェックを飛ばして type 決定に進む | Step 2 は省略しない。検出時は Step 2 の区分に従う（1・2・4 は中止、3 は該当行をユーザーへ提示して確認する） |
