...
| 11 | — | 実状態を再取得したところ、**第三者が別の親を設定済み**だったため補償せず見送った（`reason=third-party-parent`）。次のいずれかから到達する: (a) DELETE 後の POST 失敗を受けての再取得、(b) DELETE 失敗そのものを受けての再取得（Issue #489。DELETE の応答だけで exit 3 と断定せず再取得した結果、第三者の親配下と判明した場合）。**新親配下の場合はこの判定より先に成功経路（exit 0 / result=reassigned）へ倒す**ため、この exit 11 に到達するのは実測が旧親でも新親でもない別の親だった場合に限る（codex-review P2 指摘 PR #491・Issue #489） | 要確認事項へ記載。**同じコマンドで再実行してはならない。** stderr が示す実測の親をユーザーへ提示して承認を得たうえで再実行する | **変更あり**（旧親からは既に DELETE 済み。**「補償 POST を撃たず fail-closed で停止」は新親への POST 側の話であり、DELETE は exit 11 に到達する時点で既に成立している（第三者の親配下に見えている以上、サーバー側のデータモデル上、旧親からは既に外れている）。** 実測では第三者が設定した別親配下にある） |
...
- **DELETE 失敗** → 実状態を再取得し、旧親配下のまま安定確認できれば従来どおり `exit 3`
  ...
  `exit 0` にする（codex-review P2 指摘 PR #491。旧親・孤児から始まり安定確認の途中で
  新親を観測した場合は既に成功終端になっていたが、最初の読み取りが直接新親を観測した
  場合だけこの判定が抜けており、承認外の第三者親（`exit 11`）へ誤って倒れていた。
  観測タイミングだけで結果が変わる非対称の是正）。それ以外で第三者が別の親を設定済みなら
