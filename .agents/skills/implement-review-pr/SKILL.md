...
- 命名規則
- Import ルール
- フロントエンド（React 等）の変更を含む場合: 不要な再描画リスク
- テストカバレッジ
...
レビュー完了後、レポートに「①仕様準拠」「②コード品質/セキュリティ」の両セクションが記述されていることを確認する（対象リポジトリに `.claude/rules/verification.md` が存在する場合はそちらの5段階ゲートに従う。存在しない場合は次の要点に従う: 証拠なしに完了と宣言しない）。セキュリティ問題（HIGH）が残る場合は完了を宣言しない。
