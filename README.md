...
ANN opt-in 経路に積み上がった探索メモリ局所性・構築並列化・フィルタ付き探索の各施策（#486〜#507）を通しで前後比較した実測は `docs/design/hnsw-phase3-before-after.md` を参照してください（Issue #507。`bench-hnsw-compare`・`bench-knn-profile`・`feature_bench`〔hnsw opt-in〕・Recall 3 ゲート層 B を before/after で交互 N=5 ペア実測。共有 QEMU 環境の参考値・採否根拠にしない。再現手順は同 doc §9）。
