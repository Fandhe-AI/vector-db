...
| `make e2e-three-client-http` | TASK-183（HTTP-13）NoSQL 表層（`--surface nosql`）の無改造 HTTP クライアント（curl／urllib／fetch）統合テスト（`ci` には含めない opt-in。要 `curl`・`python3`・`node`（≥18）。`CURL_BIN`／`PYTHON_BIN`／`NODE_BIN` で上書き可） |
