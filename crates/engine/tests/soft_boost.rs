...
/// モジュールドキュメントが `SOFT_BOOST_PER_MATCH` について述べる「既定
/// `RrfConfig` 下で真の 1 位を上回れない」保証は、対象がプール最下位級
/// （`pool_depth` 由来の大きなスコア差）にいる最悪ケースを前提にしたものであり、
/// 本コーパスのように対象が真の 1 位のすぐ近く（密ランク 3）にいる場合には
/// 保証の前提が異なる（本値 `RANK3_TOP_K_ENTRY_BOOST` は `0.00052` を上回らない
/// よう意図的に別途選んだ値で、`SOFT_BOOST_PER_MATCH` の現在値とは無関係）。
/// 本テストは「Top-k 圏内への浮上」だけを検証したいので、密ランク 2（id=2）との
/// スコア差
