...
const ALLOWLIST: &[(&str, u32)] = &[
    ...
    // 2 件の行番号が移動したための追随（旧: 2565／2644）。Issue #905（UNIQUE
    // 制約。`UniqueConstraint`・`validate_unique_constraints`・カタログ v6
    // encode/decode・`parse_unique_section`・`alter_table_drop_column` の
    // UNIQUE 列拒否・`Storage::alter_table_add_unique_constraint`）で
    // `catalog.rs` 冒頭側に行が追加され、以下 2 件の行番号がさらに移動した
    // ための追随（旧: 2842／2921）。
    ("catalog.rs", 3189),
