...
fn rls1_no_cross_tenant_leakage_across_visibility_rates() {
    ...
    for (rate_idx, &visible_rate) in [0.9, 0.5, 0.3, 0.1].iter().enumerate() {
        ...
        for query_idx in 0..5u64 {
            ...
            let hits = index
                .search(&ctx, &storage, &CpuScalarProvider, &query, 20)
                .expect("search ok");
            for hit in &hits {
                assert!(
...
fn rls3_search_calls_provider_exactly_once_with_requested_k() {
    ...
    let hits = index
        .search(&ctx, &storage, &provider, &query, 10)
        .expect("search ok");
    ...
    let provider = CountingProvider::new();
    let hits = index
        .search(&ctx, &storage, &provider, &query, 3)
        .expect("search ok");
...
fn rls4_recall_matches_full_scan_theoretical_upper_bound() {
    ...
    for query_idx in 0..5u64 {
        ...
        let hits = index
            .search(&ctx, &storage, &CpuScalarProvider, &query, K)
            .expect("search ok");
