//! 性能計測プロトコル基盤（TASK-158）。
//!
//! `docs/spec/04-behavior/README.md` 前提条件節の性能計測プロトコルを実装として
//! 提供するモジュール群。TASK-127（SIMD 検索ベンチ）・TASK-130（GPU vs CPU-SIMD
//! A/B 回帰）・TASK-83 等、性能系の受け入れ基準を測定・再測定するタスクは、
//! 独自に計測ループを書かず必ず本モジュール経由で計測する契約とする
//! （ポインタ: `docs/spec/05-tasks.md` TASK-158）。
//!
