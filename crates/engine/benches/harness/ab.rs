...
///
/// # 戻り値の drop に関する契約（Issue #302）
///
/// `workload_a`/`workload_b` の戻り値は `black_box` を経由したのち測定区間
/// （`Instant::now()` 〜 `elapsed()`）の**内側で** drop される。戻り値がヒープ確保を
/// 伴う場合（`Vec` 等）、その解放コストは計測サンプルへ含まれる。解放コストが
/// ワークロード本体の差分に対して無視できない規模の場合（`batch_bench.rs::
/// run_core7_gate` のように、比較したい差分がマイクロ秒未満で戻り値が数百 KiB
/// 規模のとき等）、呼び出し側は測定区間外へ退避する sink（呼び出し元のクロージャに
/// キャプチャした `Vec` 等）へ戻り値を push し、`run_ab` 完了後にまとめて drop する
/// ことで解放コストを測定対象から除外する。
///
