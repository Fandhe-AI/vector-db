...
impl EngineCore {
    ...
    /// （展開結果の `mode_hint`）を優先順位に従って解決した実効モードまで含めて返す
    /// （TASK-164・PLAN-11）。`VectorCore` trait へは昇格しない固有メソッド
    /// （[`Self::plan_query`] と同じ理由。`core-api-check` の対象外）。
    ///
    ...
    /// 由来）を渡し、明示指定が常にプランナー推定より優先されることを本メソッドの
    /// 契約として保証する（`resolve_mode_with_planner` の短絡順序がその構造的根拠。
    /// `sql::mode` モジュールドキュメント参照）。展開自体の fail-closed 契約
    /// （LLM 未接続・プロンプト超過・LLM 応答不正はすべて `Err`）は [`Self::plan_query`]
    /// と共有する（モードの fail-safe とは独立したエラー系統。`query_planner.rs`
    /// モジュールドキュメント参照）。
