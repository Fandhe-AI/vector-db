...
/// 書き込み経路の統合ではなく契約の恒久化を選択した。判断の詳細・検討した代替案は
/// `docs/design/batch-ledger-scope.md` 参照）。なお `redb::Table::len()` は B-tree
/// ルートヘッダの長さフィールドを返す O(1) 操作であり、「混在検出には DB 全体の
/// スキャンを要する」という当初の想定は不正確だった。検出コストではなく、
/// 保護対象が製品書き込み経路（`catalog.rs` の per-table 行テーブルはバッチ台帳の
/// 対象外）ではなくテスト・ツール経路に留まること、台帳累計を保持する追加メタキーの
/// 管理コスト、`ROWS_TABLE` に削除 API が追加された際の誤検出リスクを理由に、
/// O(1) 検出を導入しない判断をした（同 ADR 参照）。MS-5 の障害回復契約が要求する
/// 台帳は `operation_id` 台帳（TASK-93）であり、本テーブルとは別系統である。
///
