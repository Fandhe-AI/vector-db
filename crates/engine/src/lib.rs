...
//! 差し込める（詳細は `gpu_batch.rs` モジュールドキュメント参照）。Issue #542
//! では `gpu_batch::packed_i8::GpuI8BatchBackend` として i8 パック常駐・
//! `dot4I8Packed` シェーダによる opt-in 候補生成経路（最終スコアは f32 再計算。
//! primary へは未接続）を追加した（詳細は `gpu-batch-i8-packed.md` 参照）。
//!
