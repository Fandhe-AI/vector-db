...
impl<'a> ScalarRef<'a> {
    ...
    /// GROUP BY キー等）が `Bool`／`Bytes`／`Enum` を取り違えて TEXT として扱わないよう、
    /// `Text` 以外は `None` を返す（fail-closed。呼び出し元はスキーマ型で
    /// 事前に対象外の列を除外するか、`None` を型不一致として拒否する）。
    pub fn as_text(&self) -> Option<&'a str> {
        match self {
            ScalarRef::Text(s) => Some(s),
            ScalarRef::Bool(_) | ScalarRef::Bytes(_) | ScalarRef::Enum(_) => None,
        }
    }
    ...
    pub fn as_bool(&self) -> Option<bool> {
        match self {
            ...
            ScalarRef::Text(_) | ScalarRef::Bytes(_) | ScalarRef::Enum(_) => None,
        }
    }
    ...
    /// `Bool`／`Bytes` は対象外のまま `None`（TABLE-14 が定める述語の範囲を
    /// 超えて ENUM を露出しない）。
