...
//! `Connection: close` を常に固定で付ける。`Date` ヘッダは RFC 9110 上 SHOULD で
//! あり MUST ではないため、`http::status`／`http::error_body` と同様に時刻・外部
//! 状態に依存しない決定的出力を優先し、本モジュールでは付与しない。
//!
...
use crate::http::{error_body, status::http_status};
...
/// ステータス行の固定オーバーヘッド見積り（ステータス行＋3 ヘッダ行＋空行の
/// 定数部分。`body`／`Content-Length` の桁数を除く上限バイト数）。事前確保の
/// 目安値であり超過しても正しさには影響しない。
...
/// `status`／`reason`（呼び出し元が確定済みの組）＋`body` から応答バイト列を
/// 組み立てる共有実体（infallible）。[`encode`]・[`encode_ok`] の双方が委譲する。
///
...
/// <body>
/// ```
fn encode_known(status: u16, reason: &'static str, body: &str) -> Vec<u8> {
...
pub fn encode(status: u16, body: &str) -> Result<Vec<u8>, ResponseEncodeError> {
    match reason_phrase(status) {
        Some(reason) => Ok(encode_known(status, reason, body)),
...
pub fn encode_error(class: ErrorClass, message: &str) -> Vec<u8> {
    let status = http_status(class);
    let body = error_body::encode(class, message);
    encode(status, &body).unwrap_or_else(|ResponseEncodeError| {
        encode_known(
            500,
...
pub fn encode_error_may_be_committed(class: ErrorClass, message: &str) -> Vec<u8> {
    let status = http_status(class);
...
{}"
    ...
    fn encode_error_golden_bytes() {
        let bytes = encode_error(ErrorClass::InternalError, "internal error");
        assert_eq!(
            bytes,
...
{\"error\":{\"wire_code\":\"XX000\",\"code\":\"INTERNAL_ERROR\",\"message\":\"internal error\"}}"
    ...
    fn all_error_classes_produce_well_formed_responses() {
        for class in ErrorClass::ALL {
            let normal = encode_error(class, "boom");
            assert_well_formed(&normal);

            let emergency = encode_error_may_be_committed(class, "boom");
    ...
    fn encode_error_matches_manual_composition_for_all_classes() {
        for class in ErrorClass::ALL {
            ...
            let expected_normal = encode(status, &expected_normal_body)
                .expect("status must be within reason_phrase table");
            assert_eq!(
                encode_error(class, "msg"),
                expected_normal,
                "class={class:?}"
            );

            let expected_emergency_body = error_body::encode_may_be_committed(class, "msg");
            let expected_emergency = encode(status, &expected_emergency_body)
                .expect("status must be within reason_phrase table");
            assert_eq!(
                encode_error_may_be_committed(class, "msg"),
                expected_emergency,
                "class={class:?}"
            );

            // 通常応答の本文には緊急応答専用の `data` が現れない。
            let normal_body_bytes = &encode_error(class, "msg")[..];
            let normal_text =
    ...
    fn encode_rejects_unknown_status_codes() {
        assert_eq!(encode(999, "{}"), Err(ResponseEncodeError));
        assert_eq!(encode(0, "{}"), Err(ResponseEncodeError));
        assert_eq!(encode(418, "{}"), Err(ResponseEncodeError));
    }
    ...
    fn encode_is_deterministic() {
        for class in ErrorClass::ALL {
            assert_eq!(
                encode_error(class, "x"),
                encode_error(class, "x"),
                "class={class:?}"
            );
            assert_eq!(
                encode_error_may_be_committed(class, "x"),
                encode_error_may_be_committed(class, "x"),
                "class={class:?}"
            );
        }
        assert_eq!(encode_ok("{}"), encode_ok("{}"));
    }
