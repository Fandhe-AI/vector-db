...
/// （`explain: true` の `search` を含む）以外、または該当 op でも `engine`
/// 未接続）の要求に返す暫定応答の文言（`Op::Insert` の結線は #771 の担当。
/// 本 Issue 時点は `scan`・`aggregate`・`search`（`explain` 判定を含む）の
/// 3 op のみ seam を置き換え済み）。
