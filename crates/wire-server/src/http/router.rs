...
//! ディスパッチし、`/v1/session`・`/v1/session/close` 以外は従来どおり
//! [`crate::http::conn::PlaceholderRouter`] と同じバイト列（`08P01`）で拒否
//! する（`/v1/query` の実ディスパッチは Issue #758 が本ルータへ追記する）。
//!
...
impl RequestHandler for Router {
    fn handle(&self, req: &Request<'_>) -> Vec<u8> {
        ...
        // `/v1/query` を含む未対応パスは、Issue #747 時点の `PlaceholderRouter`
        // と同一のバイト列で拒否する（実ディスパッチは #758 が担う）。
