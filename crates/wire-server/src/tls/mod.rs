...
//!   ドキュメンテーションコメントを参照）
//!
//! HKDF／鍵スケジュール（#956）・alert の実送出やハンドシェイク状態機械
//! （#965）・レコード保護／暗号化（#959）・接続への結線（#966 以降）は
//! いずれも後続 sub-issue の担当であり、本モジュールは対象外のまま。
//! 並列開発時のコンフリクトを避けるため、後続 sub-issue は `pub mod` を
//! 1 行ずつ追加していく想定。
