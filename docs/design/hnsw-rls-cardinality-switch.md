...
- **I2: リンク非参照（実装上の不変条件。本 Issue でゲート下の緩和対象）** —
  ...
  意味での**他テナントの不可視行**を指す。per-`(table, ctx)` 索引（#409。
  索引は ctx 可視アリーナのみから構築）では他テナント行がそもそもグラフに
  存在しないため、I2 はテナント境界そのものではなく、その上に置かれた
  「探索経路の単純化」という実装上の選択だったと整理できる
...
| 論点 | 整理 |
| ---- | ---- |
| 他テナント行 | 索引は `(table, ctx)` キーで ctx 可視アリーナのみから構築（#409）。非受理ノードとして現れるのは (a) 構築後に失効した stale ノード、(b) `WHERE` で除外された ctx 可視行、の 2 種のみ。ACORN-1 が読むリンクはいずれも「この ctx が構築時点で可視だった行」の隣接リストであり、他テナント行の存在・内容・近傍情報は経路上に構造的に存在しない |
| stale ノードのうち「不可視化」（構築後に別テナントへ再割当された行）の subcase | 索引が保持するリンクは構築時点（ctx がまだその行を可視として持っていた時点）の旧ベクトル近傍を符号化したものであり、再割当後の新ベクトル・新テナントの情報は索引に含まれない。ctx は既にその行を検索可能だった時点の情報を再利用するだけであり、再割当後のテナントへの横断経路は生じない。stale 比率は `REBUILD_DELTA_RATIO`（既定 1/10）超で再構築されるため古さは有界 |
...
| ADR 本文との関係 | 「非可視ノードを探索経路として通過させる設計は不採用」の「非可視ノード」＝ctx にとっての不可視行（他テナント行）であり、per-ctx 索引にはそもそも存在しない。ADR 本文は無変更のまま、本節がその解釈を記録する |
