...
- 対応: TASK-106（MS-2 / phase:2・並行フォローアップ、実装をブロックしない独立タスク）
...
TASK-106 は spec 上「データセット選定は人間・実測は Claude Code」の共同担当タスク
だが、自動運転モードで人間の選定を待てないため、**リポジトリ内で決定的に生成する
合成日本語主体コーパスによる暫定測定**として実施した（実コーパスでの再実測は
「制約・スコープ外」参照）。
...
- **ASCII のみ構成は Recall 0**。本コーパスは日本語主体（内容語・クエリとも CJK
  文字列）であり、CJK トークンを一切使わない構成では内容語を全く拾えないことが
  実測で裏付けられた。これは事前の期待方向と一致する
- 以上より、本実測の範囲では **CJK トークナイザ（ユニグラム・バイグラム）の使用
  自体が日本語主体コーパスでの検索品質を support する上で必須**であり、ストップ
  ワード除去の ON/OFF は本 QA 設計（内容語 AND 一致クエリ）に対しては軽微な差に
  留まる（Proposed）。ただし本 QA セットは `direct` カテゴリのみであり、助詞・
  機能語自体を問うクエリや、内容語 1 語のみの曖昧なクエリでの挙動は範囲外である
