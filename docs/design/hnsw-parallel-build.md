...
- ~~100k 点フルラダーでの導入前後比較~~ → Issue #450 追記（下記）で実施済み
  （`repair` 段が threads=12 で約 12.5 倍短縮し受け入れ条件を満たすことを
  確認）。重複ヘビーコーパスでの C 単独の速度実測は引き続き未実施のまま
...
増分は before 平均 17 → after 平均 1 で **約 1/17（≈0.059）まで縮小**し、
**#448 の受け入れ条件「並列由来増分が導入前比 1/10 以下」を満たす**
（100k 点フルラダーでの最終判定。#448 追記時点は 100k 点実測が
運用者実測待ちだったが、本節で確定した）。
...
実測（約 7〜9 倍）と同水準以上の削減が確認でき、**#449 の受け入れ条件
「12 スレッドで repair 段短縮」を満たす**。100k 点は #449 実測時点の
...
1. **#448（発生抑制）は 100k 点フルラダーでも有効**——並列由来の
   到達不能ノード増分が before 平均 17 → after 平均 1（約 1/17）へ縮小し、
   受け入れ条件（1/10 以下）を明確に満たした。
2. **#449（BFS ビットマップ化・冗長 BFS 省略・最近傍探索並列化）は
   100k 点・実際に修復対象ノードが発生する条件でも有効**——`repair`
   （外側）段が threads=12 で約 12.5 倍（816ms→65ms）に短縮した。
