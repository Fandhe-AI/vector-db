...
各点 warmup 20 回・計測 20 回の中央値を 2 回実測（run3・run4。
2026-09-05）。環境は「受け入れ条件 (b)」と同一の QEMU ゲスト
（`lscpu`: `Thread(s) per core: 1`・1 ソケット 12 コア・L3 16 MiB・
NUMA 1 ノード、x86_64 AVX2+FMA）。run3 は loadavg 約 1.9〜6.0（比較的
静かな状態）、run4 は threads=1 計測時のみ他ジョブと並走し loadavg
約 8.8（threads=2 以降は 2.1〜7.2 まで低下）。

#### 段別内訳（run3）

| threads | total | level | prefix | parallel | freeze | repair | serial_share | parallel_speedup | total_speedup |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 9,878.3 ms | 0.000 ms | 9,868.6 ms | 0.000 ms | 0.000 ms | 0.000 ms | 99.90% | 1.000x | 1.000x |
| 2 | 5,298.7 ms | 1.072 ms | 6.023 ms | 5,035.3 ms | 0.704 ms | 232.291 ms | 4.53% | 1.000x | 1.864x |
| 4 | 3,072.7 ms | 1.078 ms | 6.017 ms | 2,608.3 ms | 0.680 ms | 443.196 ms | 14.68% | 1.931x | 3.215x |
| 8 | 2,102.2 ms | 1.078 ms | 6.005 ms | 1,444.8 ms | 0.626 ms | 636.130 ms | 30.63% | 3.485x | 4.699x |
| 12 | 2,098.9 ms | 1.086 ms | 6.006 ms | 1,253.2 ms | 0.623 ms | 819.496 ms | 39.41% | 4.018x | 4.706x |

#### 段別内訳（run4）

| threads | total | level | prefix | parallel | freeze | repair | serial_share | parallel_speedup | total_speedup |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 10,694.3 ms | 0.000 ms | 10,791.2 ms | 0.000 ms | 0.000 ms | 0.000 ms | 100.91%\* | 1.000x | 1.000x |
| 2 | 6,018.2 ms | 1.075 ms | 6.006 ms | 5,720.4 ms | 0.711 ms | 283.511 ms | 4.84% | 1.000x | 1.777x |
| 4 | 3,323.8 ms | 1.079 ms | 6.015 ms | 2,783.1 ms | 0.703 ms | 511.878 ms | 15.63% | 2.055x | 3.217x |
| 8 | 2,059.0 ms | 1.082 ms | 6.002 ms | 1,445.1 ms | 0.551 ms | 597.864 ms | 29.41% | 3.958x | 5.194x |
| 12 | 2,071.7 ms | 1.062 ms | 6.016 ms | 1,252.9 ms | 0.716 ms | 806.436 ms | 39.30% | 4.566x | 5.162x |

\* run4 の threads=1 点は他ジョブ並走（loadavg 8.8）の高負荷下で計測しており、
`prefix` の中央値が `total` の中央値をわずかに上回る（`total`・`prefix` は
別々の 20 回分布から中央値を取るため、高負荷下ではこの逆転が起こり得る）。
threads=1 の絶対値は静かな環境の run3 を主として参照する。
...
| --- | --- | --- | --- | --- | --- |
| 2 | 49,487/50,257/50,257 | 5,051.706/5,051.715/5,051.715 ms | 0.00% | 0.366/0.192 ms | 0.00% |
| 4 | 24,679/24,973/25,119 | 2,607.939/2,607.984/2,608.055 ms | 0.01% | 1.449/0.431 ms | 0.01% |
| 8 | 7,254/13,293/13,433 | 1,452.785/1,453.006/1,453.084 ms | 0.01% | 6.057/0.893 ms | 0.05% |
| 12 | 7,247/8,324/9,277 | 1,259.523/1,260.372/1,260.564 ms | 0.02% | 446.850/56.928 ms | 2.95% |

#### ワーカー統計（run4）

| threads | inserted[min/med/max] | busy[min/med/max] | lock_blocked | lock_wait[sum/max] | lock_wait_share |
| --- | --- | --- | --- | --- | --- |
| 2 | 49,821/49,923/49,923 | 5,661.127/5,661.182/5,661.182 ms | 0.00% | 0.460/0.301 ms | 0.00% |
| 4 | 24,858/24,987/25,011 | 2,729.794/2,729.842/2,729.996 ms | 0.01% | 1.518/0.433 ms | 0.01% |
| 8 | 7,228/13,285/13,327 | 1,447.287/1,447.433/1,447.665 ms | 0.01% | 5.372/0.801 ms | 0.05% |
| 12 | 7,230/8,278/9,308 | 1,250.216/1,252.311/1,252.584 ms | 0.02% | 459.305/52.963 ms | 3.06% |
...
| threads | run3 dot_scan median | run3 speedup_ref(基準 threads=1・参考) | run3 parallel_vs_control(基準 threads=2) | run4 dot_scan median | run4 speedup_ref(基準 threads=1・参考) | run4 parallel_vs_control(基準 threads=2) |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 56.524 ms | 1.000x | — | 69.074 ms | 1.000x | — |
| 2 | 26.855 ms | 2.105x | 1.000 | 28.064 ms | 2.461x | 1.000 |
| 4 | 12.303 ms | 4.595x | 0.884 | 11.762 ms | 5.873x | 0.861 |
| 8 | 7.806 ms | 7.241x | 1.013 | 7.607 ms | 9.081x | 1.073 |
| 12 | 6.062 ms | 9.324x | 0.907 | 6.073 ms | 11.374x | 0.988 |
...
1. **主因は `repair_reachability`（凍結後・単一スレッドの後始末）**。
   2→4→8→12 スレッドで 232→443→636→819 ms（run3）／
   284→512→598→806 ms（run4）と単調増加し、12 スレッドでは total の
   39% を占める。並列度が上がるほど上位層の到達不能ノードが増え
   修復量が増える構造（`hnsw.rs::repair_reachability` は各反復で
   BFS と到達済み全ノードとの dot 計算を伴う。反復上限
   `PRECISE_REPAIR_CAP` は本タスクの実装既定値）。8→12 の差分は
   run3・run4 とも並列段の短縮（約 −192 ms）を repair の増加
   （run3 約 +183 ms・run4 約 +209 ms）がほぼ相殺し、total は横ばい
   （run3: 2,102.2→2,098.9 ms・run4: 2,059.0→2,071.7 ms）。計測方法
   修正後の再実測 2 回とも「8→12 で伸びない」現象が再現した。
...
3. **ロック競合は試行回数ベースでは軽微だが、累積待ち時間で見ると
   ...
   Σ5〜6 ms（busy 比 0.05%）に対し、12 スレッドでは Σ447〜459 ms
   （busy 比 2.95〜3.06%・ワーカー最大待ち時間 53〜57 ms）へ急増し、
   12 スレッドで初めて無視できない競合が現れる。ただし並列段全体
   （約 1,250 ms）の 3% 程度に留まり、主因ではない。
4. **並列段は対照負荷にほぼ追随している**。同一基準（threads=2）で
   正規化した `parallel_vs_control` は 4/8/12 スレッドで
   0.88/1.01/0.91（run3）・0.86/1.07/0.99（run4）とおおむね 1.0 に
   収まる。参考値（threads=1 基準の `speedup_ref`）では、12 スレッドの
   対照負荷 speedup は run3 で 9.3x・run4 で 11.4x（run4 の threads=1 点は
   高負荷下で遅く計測されているため過大評価。所見末尾の注記参照）
   であり、12 vCPU の天井に対し並列段自体はほぼ追随していると言える。
5. **ワーカー間の挿入件数に偏りがあるが busy 時間はほぼ揃う**。
   8 スレッドで min 約 7.2k・max 約 13.4k、12 スレッドで
   7.2k〜9.3k の範囲に偏るのに対し、busy はワーカー間でほぼ等しい
...
注記: run4 の threads=1 点は他ジョブ並走による loadavg 8.8 の高負荷下の
計測であり、`prefix` 中央値が `total` 中央値をわずかに上回る（別々の
20 回分布から独立に中央値を取るため）。threads=1 の絶対値・比較は
run3（loadavg 約 1.9）を主として参照する。
