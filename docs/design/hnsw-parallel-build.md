...
1. **主因は `repair_reachability`（凍結後・単一スレッドの後始末）**。
   ...
   `PRECISE_REPAIR_CAP` は本タスクの実装既定値）。8→12 の差分で見ると
   1 回目は並列段の短縮（1,436→1,236 ms＝約 −199 ms）を repair の増加
   （約 +183 ms）が相殺して total が横ばいになった一方、2 回目は
   8 スレッド点が遅く（total 2,630 ms・loadavg 4.3）8→12 で 1.23x
   改善しており「8→12 で伸びない」現象自体は再現していない。4 標本
   （「受け入れ条件 (b)」の 2 回を含む）で 12 スレッドの total は
   2,090〜2,139 ms と安定し、8 スレッドは 2,080〜2,630 ms とばらつく
   ため、8→12 の差分は 8 スレッド点のノイズに左右される。頑健な結論は
   「12 スレッドの total 約 2.1 s の内訳は repair 0.8〜0.9 s（約 40%）＋
   並列段 1.24〜1.47 s であり、並列段は対照負荷の天井（所見 4）に
   張り付いている」という構成比の説明である。
