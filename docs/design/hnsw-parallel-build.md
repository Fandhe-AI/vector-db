...
`make bench-hnsw-parallel-build`（本開発環境・12 論理コア・NVIDIA GeForce
RTX 3060 は本ベンチでは不使用・CPU のみ）で rows=100,000・dim=64・既定
パラメータの実測（本セッションでは共有計測環境の負荷により
`threads=[1, 4, 8, 12]` ラダーの全点計測が完了せず、`threads=1` の基準点
のみ実測できた。残りのラダー点の実測は運用者が `make
bench-hnsw-parallel-build` を承認済み計測環境で実行して補うことを申し送る）:

| threads | median | speedup |
| --- | --- | --- |
| 1 | 11,707.7 ms | 1.000x |

小規模スモーク実測（rows=5,000・dim=64。スレッド数ラダー確認用の追加計測。
こちらは全点計測を完了している）:
...
小規模実測でスレッド数に応じて構築時間が短縮することを確認した（受け入れ
条件 (b)）。100k 点での完全なラダー実測（`threads=4/8/12` を含む）は上記の
とおり運用者作業として申し送る。ロック競合・`repair_reachability`（凍結後の
単一スレッド後始末）が並列度を頭打ちにする程度は、100k 点の完全なラダー
実測が揃った時点で運用者が本節へ追記することを想定する（並列フェーズ自体は
速くなるが、`repair_reachability` は本タスクの範囲外のまま単一スレッドで
残るため、スレッド数を増やすほど `repair_reachability` の相対的な比重が
増える見込み）。
