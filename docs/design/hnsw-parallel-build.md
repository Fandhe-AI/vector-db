...
- BFS 本体の並列化（frontier 同期方式・原子ビットマップ）: Issue の
  ...
  判断する材料として申し送る。
- ~~100k 点フルラダーでの導入前後比較~~ → Issue #450 追記（下記）で実施済み
  （共有環境・N=2 ペアの参考計測で `repair` 段が threads=12 で約 12.5 倍
  短縮する傾向を確認。受け入れ条件の確定判定は
  `docs/design/benchmark-judgement-policy.md` §3 準拠の N≥5 再実測待ち）。
  重複ヘビーコーパスでの C 単独の速度実測は引き続き未実施のまま
  運用者による専有環境での追加実測へ申し送る。

### Issue #450 追記（2026-09-07）: repair 逐次段削減（#447〜#449）の前後比較実測・Recall 非劣化検証

Issue #447（観測フック・修復対象ノード数の可視化）・#448（発生抑制）・#449（`repair_reachability` の逐次コスト削減）の効果を、100k 点フル
ラダーで before（`f3a2019`。#447 適用前）／after（本 doc 追記時点の
`HEAD`＝`2fc0a52`。#447〜#449 適用後）の前後比較として実測した。
`Cargo.lock` は `f3a2019`〜`HEAD` で完全同一（`git diff --quiet` で
確認済み。`docs/design/benchmark-judgement-policy.md` §3 の前提を満たす）。

**計測条件**: `BENCH_HNSW_PARALLEL_THREADS=1,2,4,8,12
make bench-hnsw-parallel-build`、rows=100,000・dim=64・既定パラメータ、
各点 warmup 20 回・計測 20 回の中央値を before／after 交互 2 回実測
（before-run1 → after-run1 → before-run2 → after-run2。2026-09-07）。
環境は上記「Issue #406 追記」・「Issue #447 追記」と同一の QEMU ゲスト
（12 vCPU・AVX2+FMA・AVX-512 なし）だが、他 Issue の並列実装が同時実行中
で loadavg が 2.0〜12.8 の範囲で変動した（各 run 開始時・各 threads 点の
`noise` 行に実測 loadavg・rss を記録。**共有環境・N=2 ペアのため
`docs/design/benchmark-judgement-policy.md` §3 の N≥5 に満たず、本節の
数値は参考値・傾向確認に留め、専有環境での N≥5 再実測をオーナー作業へ
申し送る**）。

#### 段別内訳（before・`f3a2019`）

| threads | total（run1） | total（run2） | repair（run1） | repair（run2） | serial_share（run1） | serial_share（run2） | total_speedup（run1） | total_speedup（run2） |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 9,783.392ms | 9,863.508ms | 0.000ms | 0.000ms | 99.98% | 99.98% | 1.000x | 1.000x |
| 2 | 5,231.060ms | 5,254.346ms | 223.691ms | 230.187ms | 4.43% | 4.53% | 1.870x | 1.877x |
| 4 | 3,032.155ms | 3,046.460ms | 438.324ms | 444.105ms | 14.72% | 14.84% | 3.227x | 3.238x |
| 8 | 2,077.119ms | 2,048.843ms | 637.235ms | 591.780ms | 31.06% | 29.27% | 4.710x | 4.814x |
| 12 | 2,048.535ms | 2,078.141ms | 810.831ms | 822.004ms | 39.97% | 39.94% | 4.776x | 4.746x |

`f3a2019` は #447 以前のため `repair_unreachable` 系の観測フック自体が
存在せず、`repair`（外側の `HnswBuildProfile.repair_reachability`）のみ
記録される。

#### 段別内訳（after・`HEAD`＝`2fc0a52`）

| threads | total（run1） | total（run2） | repair（run1） | repair（run2） | flatten（run1） | flatten（run2） | serial_share（run1） | serial_share（run2） | total_speedup（run1） | total_speedup（run2） |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 9,591.259ms | 9,451.359ms | 0.000ms | 0.000ms | 0.000ms | 0.000ms | 99.97% | 99.97% | 1.000x | 1.000x |
| 2 | 5,198.949ms | 5,084.794ms | 58.958ms | 58.054ms | 7.725ms | 7.556ms | 1.43% | 1.45% | 1.845x | 1.859x |
| 4 | 2,758.594ms | 2,699.491ms | 66.097ms | 61.954ms | 7.156ms | 7.263ms | 2.94% | 2.86% | 3.477x | 3.501x |
| 8 | 1,573.168ms | 1,542.313ms | 58.065ms | 61.143ms | 8.192ms | 7.501ms | 4.71% | 4.96% | 6.097x | 6.128x |
| 12 | 1,871.522ms | 1,359.176ms | 77.407ms | 53.408ms | 8.867ms | 7.935ms | 5.03% | 5.10% | 5.125x | 6.954x |

threads=12 の `total`（run1 1,871.522ms／run2 1,359.176ms）は他 run より
run-to-run 差が大きい（run2 開始時 loadavg 6.13 に対し run1 は
threads=12 到達時点で loadavg 12.26 まで上昇していた影響と見られる。
「対照負荷（`parallel_vs_control`）」の項も参照）。`repair`・`serial_share`
は両 run とも一貫して低い水準に収まっており、この run-to-run 差は
`repair` 段自体の悪化ではなく並列挿入フェーズ側のノイズに起因する。

#### repair 統計（after）と #448 受け入れ条件「並列由来増分 1/10 以下」の判定

after 側の `repair_unreachable_sum`（修復対象ノード数の中央値。
`crates/engine/tests` 非依存の生ログから抽出）:

| threads | run1 med | run2 med |
| --- | --- | --- |
| 1 | 9 | 9 |
| 2 | 10 | 10 |
| 4 | 11 | 11 |
...
「Issue #447 追記」節の基線（run7・run8。#448 適用前）は threads=1
median 9・threads=12 median 27（run7）／25（run8）だった。並列由来の
増分（threads=12 − threads=1）を before／after で比較する:

| | before（#447 run7／run8 基線） | after（本節 run1／run2） |
| --- | --- | --- |
| threads=1 med | 9 | 9 |
| threads=12 med | 27／25 | 10／10 |
| 増分（12−1） | 18／16 | 1／1 |

増分は before 平均 17 → after 平均 1 で **約 1/17（≈0.059）まで縮小**する
傾向を確認した（100k 点フルラダーでの参考計測。共有環境・N=2 ペアのため
`docs/design/benchmark-judgement-policy.md` §3 の N≥5 に満たず、
`#448` の受け入れ条件「並列由来増分が導入前比 1/10 以下」の確定判定は
専有環境での N≥5 再実測をオーナー作業へ申し送る）。

#### `repair`（外側）段の削減率（#449 の受け入れ条件「12 スレッドで repair 段短縮」の判定）

| threads | before 平均 | after 平均 | 比（after/before） | 削減率 |
| --- | --- | --- | --- | --- |
| 2 | 226.94ms | 58.51ms | 0.258x | 約 3.9 倍 |
| 4 | 441.21ms | 64.03ms | 0.145x | 約 6.9 倍 |
| 8 | 614.51ms | 59.60ms | 0.097x | 約 10.3 倍 |
| 12 | 816.42ms | 65.41ms | 0.080x | 約 12.5 倍 |

100k 点フルラダーでも「Issue #449 追記」節の 30k 点・一様乱数コーパス
実測（約 7〜9 倍）と同水準以上の削減傾向が参考計測で確認できた
（共有環境・N=2 ペアのため確定判定ではない。`#449` の受け入れ条件
「12 スレッドで repair 段短縮」の確定判定は専有環境での N≥5 再実測を
オーナー作業へ申し送る）。100k 点は #449 実測時点の
`repair_unreachable_sum=0`（一様乱数コーパス限定の特殊状態）とは異なり
実際に数件〜十数件の修復対象ノードが発生する条件のため、A（BFS の
ビットマップ化）・B（冗長 BFS 省略）に加え C（最近傍探索の並列化）も
threads>=2 で実際に発火している（`phase1_iters` が全 threads 点で
9〜13 の範囲、`phase2_nodes` は全点で 0 のままフェーズ 2 は本条件でも
未発火）。

#### 対照負荷（`parallel_vs_control`）

`parallel_vs_control`（並列段の相対 speedup ÷ 対照負荷 `dot_scan` の
相対 speedup。基準 threads=2）は before・after とも 12 スレッドで
0.88〜0.96 のレンジに収まり、並列挿入フェーズ自体のスケーリング特性は#449 適用前後で変わっていない（`repair` 削減が並列フェーズの効率を
悪化させる副作用を生んでいないことを裏付ける）。after run1
threads=4 の `parallel_vs_control=4.083` は対照負荷 `dot_scan` の
median が同時点で 65.176ms（他 3 run は 11〜13ms 台）と突出した
外れ値であり、共有環境のノイズ（他 worktree の負荷変動）と判断する
（`repair`・`total` 等・他の指標には対応する異常は見られない）。

#### 所見

1. **#448（発生抑制）は 100k 点フルラダーでも有効な傾向**——並列由来の
   到達不能ノード増分が before 平均 17 → after 平均 1（約 1/17）へ縮小した
   （共有環境・N=2 ペアの参考計測。受け入れ条件〔1/10 以下〕を満たす
   方向のシグナルだが、確定判定は専有環境での N≥5 再実測待ち）。
2. **#449（BFS ビットマップ化・冗長 BFS 省略・最近傍探索並列化）は
   100k 点・実際に修復対象ノードが発生する条件でも有効な傾向**——
   `repair`（外側）段が threads=12 で約 12.5 倍（816ms→65ms）に短縮する
   結果を参考計測で確認した（同様に確定判定は N≥5 再実測待ち）。
3. **8→12 スレッドの頭打ち自体は残る**（`total_speedup` は before
   4.7〜4.8x・after 5.1〜7.0x で改善はしたが、依然として threads=8 の
   `total_speedup`〔before 4.7〜4.8x・after 6.1x〕に対し threads=12 での
   伸びは限定的）。原因は `repair` 段の縮小により相対的に見えやすく
   なった並列挿入フェーズ自体のロック競合（`lock_wait_share` が
   threads=12 で before 2.85〜3.33%・after 3.20〜4.03% と、他 threads
   点〔0.00〜0.07%〕より 1 桁大きい）と考えられ、これは #449 のスコープ
   （`repair_reachability` 単独）の外にある。
4. `serial_share`（総時間に対する逐次段の比率）は threads=12 で
   before 39.94〜39.97% → after 5.03〜5.10% まで低下し、逐次ボトル
   ネックとしての `repair_reachability` の支配的地位は解消された。

#### スコープ外・申し送り

- 8→12 スレッドの頭打ち自体（所見 3）の残存要因（並列挿入フェーズの
  ロック競合の増加）は未調査。`repair` 縮小により相対的に顕在化した
  ものであり、対策は本 Issue のスコープ外（親 #446 への申し送り事項）
- 専有環境での N≥5 交互再実測（`docs/design/benchmark-judgement-policy.md`
  §3 準拠の確定判定）: オーナー作業
- 重複ヘビー・adversarial コーパスでの C（最近傍探索の並列化）単独の
  速度実測（#449 で申し送り済みのまま未実施）
- `make bench-hnsw-compare`（usearch 対照）の before（`f3a2019`）／after
  （本節と同一の `HEAD`）実測——「Issue #406 追記」節の run7・run8（12
  スレッドで usearch 6.6x・自作 4.9x の speedup）を基準に、#447〜#449
  適用後の縮小幅を追記する計画だったが、本節作成時点の計測環境は
  複数の並行 Issue 実装・計測プロセスが同時実行中で著しく輻輳しており
  （`bench-hnsw-compare` の同名バイナリが未識別の要因で計測途中に
  終了する事象を実際に観測した）、100k 点フルラダー（1・2・4・8・12
  スレッド × self／usearch 各 40 回）は言うに及ばず、縮小版
  （1・12 スレッドのみ）実測すら安定して完走しなかった。信頼できない
  部分的数値を記載するよりも実施見送りを選び、本節を含む前後比較
  一式は `bench-hnsw-parallel-build`（自作単体・#448/#449 の受け入れ
  条件判定に必須）に限定した。usearch 対照の再実測は専有環境での
  オーナー作業へ申し送る。
