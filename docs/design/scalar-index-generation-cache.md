...
| hybrid ループ `warm_where_then_hybrid`.rss_after_warm | 63.73 / 63.91 | **55.58 / 55.67**（約 12.8% 減） | 0.8720 | 帯内（固定帯超えるが判定は参照帯優先で帯内） | 56.32 / 56.35 | 0.8834 | improved | 0.9868 |
...
1. **`warm_where_then_hybrid.rss_after_warm`**: before 63.73 MiB
   ...
   基準）は 56.32 MiB で after よりわずかに多い水準（比 0.9868）——`body` を
...
`hybrid_rrf`（crossdb self の 1 フェーズ）側の改善幅が相対的に小さい
理由を「`WHERE` を経由しないためコストを踏まない設計」に帰属させる
説明は誤りであり訂正する（codex-review 指摘）。`scripts/crossdb_bench/
self_db.py` は同一サーバー・同一接続で `vector_knn_where` →
`where_compound_count` → `hybrid_rrf` の順に実行するため、この
`hybrid_rrf` フェーズも先行する `WHERE` クエリで構築済みの
`ScalarIndex`（テーブル世代整合キャッシュ・#357/#473）がヒットした
状態で計測されており、「`WHERE` 未経由」の対照にはなっていない。

真に `WHERE` を経由しない対照は、本節冒頭で記録した `hybrid_after_where.py`
の 3 モードのうち **`hybrid`（ウォームアップなし。`ScalarIndex` 構築の
gate が発火しない対照）** であり、その生データ（本節と同一セッション
`20260908T102007Z`）を再集計すると before min/median 6200.4/6314.1µs →
after min/median 6285.0/6317.2µs（比 1.0136）・before_ref min/median
6286.6/6363.7µs → ref min/median 6345.0/6384.6µs（比 1.0093）——
いずれも参照帯（16.18%／8.64%）を大きく下回るほぼ無風の変化であり、
`ScalarIndex` を構築しない経路が閾値変更の影響を受けないという設計は
この対照によって支持される。一方、crossdb self の `hybrid_rrf` フェーズ
自体の改善幅（3.80%）は「`WHERE` を経由しないこと」の証跡としては
使えず、`warm_where_then_hybrid`（本節の主目的である「`WHERE` 実行後の
`hybrid_rrf`」の直接再現条件）の改善（9.83%）と合わせて解釈すべき
副次的な観測値にとどめる。共有 QEMU 環境の参考値であることを踏まえると、
**専有環境での再実測により両ノイズ帯を明確に超える確証が得られることを
条件に、親 #630 は close 可能**と判断する（本 Issue 単独では「方向は
確認済みだが共有環境のノイズ帯の広さにより統計的な確定判定には至らない」
という限定付きの達成に留める）。
