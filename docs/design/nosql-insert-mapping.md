...
- `bind_rows`／`bind_row`（純関数）: JSON `rows[*]` を `schema` の列順に
  対応する `BoundInsert` へ束縛する。`id` は JSON 数値のうち
  `0 <= n <= 2^53` を満たす非負整数のみ受理し、文字列形の `id` は拒否する
  （JSON の IEEE754 倍精度は `u64` 全域を安全に表現できないため）。
...
- `id` の JSON 数値表現域（`0 <= n <= 2^53`）は本リポ独自の実装既定値であり、
  `u64` 全域を表現できない制約が残る。文字列形 `id` の受理可否は spec 側の
  判断事項として申し送る。
