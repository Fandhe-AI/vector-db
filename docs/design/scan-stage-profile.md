...
- Issue #677: HNSW opt-in 時の `Subset` 形状 plain scan 縮退時委譲（Issue #676）の到達経路・レイテンシ計測（`BENCH_SCAN_PROFILE_ENGINE`／`BENCH_SCAN_PROFILE_FULL_SCAN_RATIO`）は本 doc ではなく `docs/design/hnsw-rls-cardinality-switch.md`「Issue #677」節へ記録した（本 doc の A/W 系列・I 系列は brute_force エンジン固定のまま不変。未設定時の出力はビット同一）。
