...
以下は B 案採用に伴う**本リポジトリ実装上の初期方針**であり、Phase 3 の
各タスク（#404〜#413）が着手時に参照する作業メモである。パラメータ初期値は
実装・実測の結果に応じて本節を更新しながら調整してよい。ビヘイビア・
エラー契約（`wire_code`）・決定性契約・RLS 契約・Recall 受け入れ基準の
**規範的な定義（SSOT）は private spec 側**にあり、本節の記述はそれらを
本リポジトリ側で新規に確定させるものではない。

以下のうち次の 3 項目は、spec 側で対応する TASK・ビヘイビア ID が
確定するまで**後続実装の受け入れ条件として使わない**（AGENTS.md
「spec（SSOT）との整合」。実装都合で新しい挙動を先行導入しない）:

- 1 節の不正パラメータ拒否時の `22023` 相当 `wire_code` 契約
- 3 節の決定性の実装方針（境界同点グループを保証しない方針を含む）
- 4 節の RLS 事前フィルタとの切替方針

上記 3 項目に対応する Phase 3 タスク（#407・#405・#410・#411・#409）は、
spec 側で TASK・ビヘイビア ID が確定し本節の記述がそのポインタへ
差し替わるまで着手しない。確定後、本節はそのポインタと実装上の
非契約的な詳細（パラメータ初期値・並列構築方式等）のみを記載する形へ
書き換える。3 項目以外（既定パラメータ・増分更新の運用方針等の
非契約的な実装詳細）は spec 確定を待たず Phase 3 が参照してよい
作業メモのままとする。
...
### 3. 決定性の実装方針（#405・#410・#411。正式な決定性契約は spec 側で確定）

以下は Phase 3 実装が暫定的に準拠する方針であり、正式な決定性契約は
spec 側の該当ビヘイビア定義（「spec 側への申し送り候補」節）で確定させる。
...
- 総当たりを前提とする境界同点グループ完全化
  ...
  方針とする（探索打ち切りにより未探索の同点候補が生じうるため）。既定
  エンジン経路の既存契約は不変
...
### 4. RLS 事前フィルタとの切替方針（#409・#410。正式な RLS 契約は spec 側で確定）

以下は Phase 3 実装が満たすべき安全側の実装方針であり、正式な RLS 契約
（テナント境界の定義そのもの）は spec 側の該当ビヘイビア定義（「spec 側
への申し送り候補」節）で確定させる。本節はその確定を待たず安全側
（fail-closed）に倒した本リポジトリの実装方針を記す。
...
- 可視率が閾値以上の場合も、共有の HNSW 索引（非可視ノードを含む）を
  ...
  - (a) `(table, PolicyContext)` 単位で、可視縮約集合に含まれるノードのみ
    から専用グラフを構築して探索する。可視集合はテーブル世代とともに
    変わりうるため、この専用グラフは「5. 増分更新」節の挿入 delta 契約を
    適用せず、`SqlArenaCache`／`SparseIndexCache` と同型の厳密世代一致
    キャッシュとする（現在のテーブル世代と不一致なら常に再構築、または
    直接 (b) へ縮退する）
...
既定エンジン非 RLS 経路（4 節 (a) の専用グラフを除く）の索引は
`(table, PolicyContext)` × **構築時点のテーブル世代**をキーに保持する。
`SqlArenaCache`／`SparseIndexCache` の厳密世代一致キャッシュ（現在世代と
1 でも不一致なら即失効）とは異なり、以下のとおり挿入と削除を非対称に
扱う挿入専用 delta キャッシュとする。世代不一致を無条件に許容するわけ
ではなく、不一致の内訳（挿入のみか削除を含むか）で扱いを分ける：

- **挿入（新規チャンク追加）のみによる世代進行**: グラフ構造は変更せず、
  未索引の新規ノードを brute-force delta として保持し検索時に結果を
  マージする。未索引比率（行数比）が閾値を超えるまでは再構築しない
  （閾値到達までは構築時世代のグラフをそのまま再利用してよい）
- **削除（同一パス置換・TASK-120 による旧チャンクの消滅）を含む世代進行**:
  削除ノードは可視 id 集合から外れ結果からはマスクされるが、HNSW の
  探索経路（エントリポイント・近傍リンクの入口）としては構造上残存し
  続け、他ノードの到達可能性・探索順を変え得る。マスクのみでは 3 節の
  決定性契約を維持できないため、削除を検知した時点で未索引比率の閾値を
  待たず即時に全体再構築（フル rebuild）する。閾値ベースの遅延再構築は
  挿入専用であり削除には適用しない（tombstone マスクは即時再構築までの
  暫定的な結果補正としてのみ用い、恒久的な代替とはしない）
- 索引構築失敗時は brute-force へ縮退し、結果契約は不変とする
- 世代競合時の insert 拒否は Issue #280 の fail-closed 契約を踏襲する
  （本節の delta 許容は検索側の索引再利用可否の話であり、書き込み経路の
  世代競合拒否契約とは独立に維持する）
- TASK-121 系の増分回帰へ、挿入 delta・削除即時再構築の双方について
  ANN 経路の非劣化アサーションを追加する
...
- ローカルでの `RECALL_VERBOSE`・`make` ターゲットによる手動実行は開発時の
  即時確認手段として用意するが、それのみに留めない。ANN opt-in 経路向けの
  ジョブを `recall.yml` へ追加し、`bench.yml` の `BENCH_CORE6`／
  `BENCH_CORE16`（GPU 搭載環境向け opt-in・週次 `schedule` 接続）と同方針の
  opt-in フラグ（例: `RECALL_ANN`）付きで `workflow_dispatch` と既存の
  週次 `schedule` の双方に接続し、ANN 実装・パラメータの変更に対する
  機械判定可能な退行検出を継続的に行う（#412 のタスクスコープに含める）
- 上記ジョブの閾値・environment は既存 `recall-gate` を踏襲し、非公開閾値の
  public ログ非露出運用（`RECALL_VERBOSE` opt-in・`GITHUB_ACTIONS` 下拒否）
  も同様に適用する
...
| 受け入れ基準 | 対応タスク |
| ---- | ---- |
| 既定エンジン不変・ANN は明示選択のみ | #407（`22023` エラー契約は spec 側 TASK・ビヘイビア ID 確定後に着手） |
...
| SEARCH 系 Recall ゲートを ANN 有効経路でも同一閾値で通過 | #412（`recall.yml` への機械判定可能な接続を含む。「7. Recall 受け入れ」節参照） |
...
- **Phase 3 着手前に必須（実装ガイド「実装ガイド（B 案・Phase 3）」節冒頭の
  ブロック対象。#407・#405・#410・#411・#409 は下記確定まで着手しない）**:
  「実装ガイド（B 案）」1 節の `22023` エラー契約・3 節の決定性契約・
  4 節の RLS 事前フィルタ切替契約それぞれに対応する新規 TASK・
  ビヘイビア ID を spec 側で起票・確定する
