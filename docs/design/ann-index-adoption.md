...
- 「可視 id によるマスクのみを結果に適用し、非可視ノードを探索経路として
  通過させる」方式は**採用しない**。HNSW の探索順・到達候補はグラフの入口・
  接続関係（＝非可視ノードのベクトルと接続を含む）に依存するため、結果を
  可視 id でマスクするだけでは、同一の可視データ・クエリでも他テナント
  データの有無によって発見される可視候補や順位が変わりうる。これは
  「結果へ露出してはならない」という要件だけでは防げない存在情報の漏えい
  であり、RLS 事前フィルタとして不十分（P0）
- `SearchInput.ids`（可視行集合）の件数と索引全体件数の比
  `full_scan_ratio`（初期値 0.1・実測で調整）で判定し、可視率が閾値未満なら
  可視縮約集合上の brute-force とする
- 可視率が閾値以上の場合も、共有の HNSW 索引（非可視ノードを含む）を
  そのまま探索してはならない。採用するのは次のいずれかに限る:
  - (a) `(table, PolicyContext)` 単位で、可視縮約集合に含まれるノードのみ
    から専用グラフを構築して探索する（増分更新は「5. 増分更新」節の
    キャッシュ単位と同じ `(table, PolicyContext)` × テーブル世代をキーに
    再構築する）
  - (b) (a) の専用グラフを持たない・構築コストが見合わない場合は、可視率が
    閾値以上であっても厳密検索（brute-force）へ縮退する
  - 非可視ノードを含む共有索引を探索経路として使う実装は、探索順・到達
    候補が入力（可視集合）だけで決まることを保証できる厳密探索（実質的に
    brute-force と同じ結果不変性）へ縮退する場合に限り許容する
...
- TASK-138（RLS 暗黙適用の全読み取り経路一般化検証）・
  ...
  （(a) の専用グラフ構築・(b) の縮退のいずれの経路でも、可視集合が異なる
  クエリ間で他テナントの存在情報が漏れないことを回帰で固定する）
