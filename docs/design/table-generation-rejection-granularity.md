...
までの期待再送回数は幾何分布で `E[retries] = P / (1 − P)` と近似できる（`P < 1`
である限り有限値）。上表の例示値で言えば、`P ≈ 0.01`（低頻度・T = 1 s）では期待再送
回数は 1 回未満で実務上無視できるが、`P` が 0.5 を超える領域（例: 高頻度書き込みで T
が長い場合）では期待再送回数が急増し、`P → 1` の極限で発散する。ただし、これは
「毎回の照合が独立かつ `P` が一定」という単純化した前提のもとでの期待値の挙動であり、
その前提下では再送回数は有限の期待値を持つ確率分布に従うため starvation（無限に拒否
され続ける事象）が**確率 1 で起きるわけではない**。**λ が高くクエリ側の再送でも都度
また拒否される状況が続けば、実務上は成功までの再送回数が大きくなり体感的な
starvation に近い状態になり得る**という理論上の可能性として区別して記述する。
したがって「クライアント側コストは再送 1 回に限られる」と可用性評価をまとめることは
過小評価であり、実際の運用判断では対象テーブルの λ・T の実測値から期待再送回数
（またはその急増・発散の有無）を判断材料に加える必要がある。
...
- 1 回の拒否自体は読み取り専用クエリの再送であり副作用を伴わない。ただし成功までの
  期待再送回数は λ・T に依存し 1 回に限定されない（λT が大きい領域では急増し、
  `P → 1` の極限で発散し得る。独立試行・一定 `P < 1` の前提下では starvation が
  確率 1 で起きるわけではないが、体感的な starvation に近い状態は理論上あり得る。
  「拒否頻度の見積もり」節参照）。この点は理論上の可能性として残るが、移行トリガー
  #2（承認済み計測環境での実測）で λT の運用上の許容性を確認する運用に委ねる
