...
- 新規動的テーブル `user_vecs/{table_name}`（`user_rows/{table}` と同じ命名規約・
  ...
  - **`slot_bits` の visibility ビット・header の `public_live`／`private_live`
    ...
    **等の書き込み経路が同一 txn 内で複製した派生値であり、権威（authoritative）
    ...
    - **admission 方向（無条件に保証）**: チャンク側複製の値だけを根拠に行を
      ...
      残りの複製値検査（3 点）を行い、不一致も同様に `Err` にはせず**当該 world**
      のフォールバックを発動させる（この検査は locator が既に一致確認済みの
      候補にのみ行うため world 単位のフォールバックで足りる。黙って除外もしない。
      下記「権威検証を行うタイミング」節参照）
  ...
  - KNN・`SearchTimeFilter`・`batch_search` はチャンク走査で得たスロットの
    ...
    **権威検証を行うタイミング（確定方針。以下がこのトピックの唯一の定義であり、
    ...
    `Err` からフォールバック（検出手段が locator に依存するか否かで
    tenant 単位／world 単位を使い分ける。下記「フォールバック契約」節の表参照）
    へ置き換えることで両立させる。以下が
    ...
    - **chunked（`layout: chunked`）経路の対応**: チャンク走査で得られるのは
      ...
      存在しない）
    - **フォールバック契約（採用する再設計。本節の中核）**: フォールバックの
      発動契機は複数あるが、**発動時にどこまでを破棄・置換するか（world 単位／
      tenant 単位）は「検出手段が行側 `locator`（`(chunk_no, slot)`）を参照
      するか否か」で機械的に決まる**（codex-review P1 指摘への対応。以前の
      記述は発動契機によらず一律に world 単位〔`chunk_no` フィルタ〕としていた
      が、`chunk_no` フィルタは行側 `locator.chunk_no` を根拠に絞り込むため、
      その `chunk_no` フィールド自体が破損している場合には絞り込みの根拠が
      壊れており、対象 world の権威フォールバック結果から本来含めるべき行が
      漏れる。この漏れを塞ぐため、検出手段が行側 `locator` に依存する場合に
      限り tenant 単位へ範囲を広げる）:

      | 検出手段 | 行側 `locator` を参照するか | フォールバック範囲 | 理由 |
      | -------- | -------------------------- | ------------------ | ---- |
      | ステップ 2: locator／逆参照の同一性検査の不一致・ポイントルックアップ自体の miss | 参照する（走査元 `(chunk_no, slot)` と行側 `locator` を突合） | **tenant 単位** | `chunk_no`・`slot` いずれの側の破損でも占有者を特定できなくなる。world 単位で `chunk_no` フィルタを掛けると、破損した当のフィールドを絞り込み条件に使うことになり、その行自体を除外してしまう |
      | ステップ 0/1: `world_completeness` の 3 値照合（`world_completeness.live_count` ↔ チャンク header `live_count` ↔ `popcount(slot_bits)`）の不一致・マーカーエントリ欠落・チャンク header 自体のデコード失敗 | 参照しない（チャンク側の値のみで完結する集計・デコード） | world 単位（`chunk_no` フィルタ） | 検出根拠が行側 `locator` を経由しないため、`chunk_no` フィルタの絞り込み条件自体は破損の影響を受けない |
      | ステップ 4: 残りの複製値検査（3 点。`row_ids[slot]`・tenant・`slot_bits` visibility）の不一致 | 参照しない（ステップ 2 で locator 一致は検査済みの候補にのみ行う） | world 単位（`chunk_no` フィルタ） | この検査に到達した時点で当該候補の `locator.chunk_no` は一致確認済みであり、`chunk_no` フィルタの絞り込み条件は健全 |

      **tenant 単位フォールバック（ステップ 2 の locator 不一致・miss 検出時）**:
      chunked 経路の当該 tenant について、まだチャンク走査から収集していた
      候補（他 world 分・複製値検査を通過済みのものを含む）を**すべて破棄**し、
      ...
      **world 単位フォールバック（ステップ 0/1・ステップ 4・チャンク header
      デコード失敗検出時）**: 当該 world 由来ですでにチャンク走査から収集して
      いた候補のみを破棄し、`user_rows/{table}` の `tenant_id` 前置キー範囲
      全体を走査した結果のうち、v3 header に含まれる `locator` の `chunk_no`
      が当該 world の `chunk_no` と一致する行だけを対象 world の権威結果として
      採用する（`chunk_no` が一致しない行＝同 tenant の他 world 由来の行は
      対象外。`layout: chunked` テーブルに v2 行が混在することはない不変条件
      のため、locator を欠く行が候補として現れることもない）。他 world
      （`chunk_no` が異なる行・別 `tenant_id` のチャンク）の候補・処理は
      一切変更されない。

      いずれの範囲でも、フォールバック先の row-store 範囲走査は非再帰・一発
      限り（当該 world・当該 tenant について chunked 経路へ再入しない）で
      あり、その走査自体が `Err` を返す場合（`user_rows` 側 header デコード
      失敗等、row-store 経路が本来 `Err` にする破損クラス）は、その `Err` を
      そのまま呼び出し元へ伝播する（フォールバックが新たな情報を付加しない）。
      - **「row-store 経路と同一の結果集合になる」という前提（網羅性の根拠と
        ...
        オンライン検出（world 完全性マーカー）」節のステップ 0/1 が world 単位
        フォールバックとして検出する。(B) チャンク側は無傷のまま、行自身が
        保持する `locator.chunk_no`（または `slot`）のみが破損し、行が
        指す側と home world のチャンクが指し返す側とで不一致になるケース
        （上記「フォールバック契約」冒頭の表の 1 行目に該当）——このケースは
        home world 側のチャンク・マーカーがいずれも無傷（3 値照合は一致した
        まま）であるため `world_completeness` の 3 値照合では検出できず、
        `chunk_no` フィルタ済みの世界単位フォールバックに置き換えても
        フィルタの根拠自体が破損しているため回収できない。**(B) はステップ 2
        の locator／逆参照の同一性検査（home world のチャンク走査で当該行に
        到達した時点で発火する）で検出し、tenant 単位フォールバックへ回す
        ことで回収する**（上記「フォールバック契約」冒頭の表参照）。この
        点検の恒久的な帯域外担保は引き続き T8（世界単位の帯域外整合性検証。
        「5. チャンク単位可視性ビットマップ」節）が担い、T8 はオンライン検出
        （同一 txn 不変条件・マーカー突合・tenant 単位フォールバック）が
        実装バグ等で破れていないかを運用時に確認する役割へ位置づけを改める
        （下記節参照）
      - **却下した代替案（tenant 全体を無条件に置換する契約）**: 検出手段を
        問わずすべてのフォールバック発動契機で `chunk_no` フィルタを行わず、
        同 tenant の**全 world**の既収集候補を破棄したうえで `tenant_id`
        範囲走査結果全体（フィルタなし）を常に採用する案。1 チャンクの
        locator ドリフト・破損という局所的な事象が、無関係な他 world の
        chunked 高速経路まで一律に破棄させてしまい影響範囲が不必要に拡大する
        （ステップ 0/1・ステップ 4 の検出はいずれも行側 `locator` を参照せず
        `chunk_no` フィルタの絞り込み根拠自体は健全なため、この 2 契機まで
        tenant 単位へ広げる理由がない）。**ステップ 2（locator 不一致・miss）
        の検出に限っては、`chunk_no` フィルタの絞り込み根拠そのものが破損
        しうるため、この案（tenant 全体を置換）を採用する**（同 tenant の
        全 world の既収集候補を破棄したうえで単一の範囲走査結果に置き換える
        ため、`chunk_no` フィルタなしで生じうる重複候補は生じない）。検出手段
        ごとに使い分けることで、影響範囲を必要最小限（世界単位）に保ちつつ、
        locator 自体の破損という `chunk_no` フィルタが原理的に対応できない
        クラスのみ tenant 単位で確実に回収する
      効果:
      1. **フォールバックが発動した world（tenant 単位フォールバックの場合は
         当該 tenant の全 world）については、結果集合が row-store 経路と
         同一になる**ため、可視行の誤除外・短い結果集合の返却が構造的に起きない
         （Bugbot Medium 指摘・codex-review P1 指摘への対応。ドリフトで live
         スロットが無関係な不可視行を指す場合も、その world 全体（または行側
         `locator` が破損した場合は当該 tenant 全体）を権威 range 走査へ切り
         替えるため、正しい占有者が検証されずスキップされる余地がない）。**この
         ...
         孤立行）の組み合わせにより、上記 2 種類の孤立行のいずれも次回以降の
         チャンク走査でホーム world への到達時点で必ず検出されフォールバックが
         発動するため（P1-2 是正）、孤立行のケースも実質的にこの保証の対象に
         含まれる。マーカー突合自体が破損している残存ケース（下記「マーカー
         自体が破損・欠落している場合」参照）は T8 の帯域外検証に委ねる**
      ...
      2. 各候補スロットについて `user_rows` へポイントルックアップし、
         **metadata はまだデコードせず** `decode_row_tenant_and_visibility` の
         ...
         フィールド自体を絞り込み条件に使う world 単位フォールバックでは
         回収できない。上記「フォールバック契約」節の表参照。テナントデータに
         依存しない構造的検査という性質は変わらず、`is_visible` 判定より前に
         行っても他テナントの存在情報を漏らさない。ただし帰結が `Err` では
         なくフォールバックである点が旧来の記述からの変更点）
      ...
      5. 複製値検査を通過した候補（フォールバックが発動しなかった world）だけが
         ...
         world（tenant 単位フォールバックの場合は当該 tenant の全 world）に
         ついては、この手順の代わりに row-store 範囲走査結果（同じ
    ...
    - **コストへの帰結（両経路で対称。「コスト見積り」節に反映）**: 上記
      ...
      `O(当該 world のチャンク走査)`」）。**tenant 単位フォールバック（ステップ 2
      の locator 不一致・miss 検出時）が発動した場合は、この退化コストが同 tenant
      の全 world 分に拡大する**（当該 tenant の row-store `tenant_id` 範囲走査
      コストに退化するだけでなく、それまでに同 tenant の各 world で行っていた
      チャンク走査・ポイントルックアップ分すべてが無駄になる。上限は「当該
      tenant の row-store 経路のコスト + `O(当該 tenant の全 world のチャンク
      走査)`」）。world 単位フォールバック（ステップ 0/1・ステップ 4）は発動しても
      他 world の chunked 経路には影響しない。tenant 単位フォールバックは発動
      しても他 tenant の chunked 経路には影響しない
    - **却下した代替案**:
...
- **チャンク単位・スロット単位のいずれも、`slot_bits`／header 集計値による判定は
  ...
  - **意味的不整合**（チャンク側複製と `user_rows` 側権威値の食い違い）: チャンク
    ...
    **tenant 単位**フォールバック発動。行側 `locator` を参照する検出のため
    上記「フォールバック契約」節の表に従い world 単位ではなく tenant 単位で
    回収する）で、`row_ids[slot]`・tenant・`slot_bits` visibility の
    不一致はステップ 3 の `is_visible` 判定を経てステップ 4 で行う残りの複製値
    検査（3 点。行側 `locator` を参照しないため world 単位フォールバック発動）
    で検出・フォールバック発動とする。ただし後者（ステップ 4 の
...
- **クエリ時の突合（3 値照合）**: 上記「権威検証を行うタイミング」節の統一
  ...
  この孤立行は当該 world のチャンク走査**開始時**に必ず検出され、world 単位の
  row-store 範囲走査へフォールバックする——「他スロットが整合していれば
  フォールバックが発動せず黙って欠落する」という P1-2 指摘の欠落は生じない。
  **一方、チャンク側は無傷のまま行自身の `locator.chunk_no`（または `slot`）
  のみが破損するケース（上記「フォールバック契約」節の表・「「row-store
  ...
  検出し、tenant 単位フォールバックへ回すことで回収する（「開始時」ではなく
  「当該行の候補到達時」に検出される点が本マーカーとは異なる）。この 2 つの
  検出経路（マーカーによる world 単位・locator 検査による tenant 単位）を
  組み合わせて初めて、孤立行の 2 クラスいずれについても「他スロットが
...
| `crates/engine/tests/power_loss.rs` | `StorageBackend` 差し替え電源断モデル | 部分書き戻し像でのチャンク値と locator の不整合を検出し、フォールバックすること（`Err` にはならず、結果が row-store 経路と一致すること）の確認。`row_ids[slot]` ⇔ `user_rows` 側 `locator` の相互不整合（再パック中の電源断を含む。行側 `locator` を参照するステップ 2 の検出のため **tenant 単位**フォールバックし結果が row-store 経路と一致することを確認する）に加え、`slot_bits` visibility ⇔ `user_rows` 側 tenant／visibility の不整合（権威検証。上記「権威検証を行うタイミング」節参照。行側 `locator` を参照しないステップ 4 の検出のため **world 単位**フォールバックで足りることを確認する）も同一のフォールバック検出対象に含める。`user_rows` 側 header デコード失敗（row-store 経路でも `Err` になる破損）のみ引き続き fail-closed で `Err` とする |
| `crates/engine/tests/index_failure_injection.rs`（RECOVER-9） | 再構築処理の途中失敗注入 | チャンク再パック処理の途中失敗で commit 済み行が無傷であることの確認。途中失敗後も `row_ids` と `locator` が旧配置のまま一貫していること（半端な書き換えが残らないこと）、および `slot_bits` visibility と行側 tenant／visibility の一致が崩れていないことを含める。`row_ids`／行側 `locator` の一致が崩れた状態を注入した場合は tenant 単位フォールバック、`slot_bits` visibility と行側 tenant／visibility の一致が崩れた状態を注入した場合は world 単位フォールバックが発動し、いずれも結果が row-store 経路と一致すること（`Err` にならないこと）を検証対象に加える |
