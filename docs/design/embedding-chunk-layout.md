...
- 新規動的テーブル `user_vecs/{table_name}`（`user_rows/{table}` と同じ命名規約・
  ...
  - **`slot_bits` の visibility ビット・header の `public_live`／`private_live`
    ...
    **等の書き込み経路が同一 txn 内で複製した派生値であり、権威（authoritative）
    ...
    - **admission 方向（無条件に保証）**: チャンク側複製の値だけを根拠に行を
      ...
      占有者を特定できない状態であるにもかかわらず `Err` にはしない**（当該
      world = `(tenant_id, chunk_no)` の chunked 高速経路を放棄し、`user_rows`
      側の権威範囲走査〔`arena.rs::build_filtered_with_rows_and_limits_in_txn`
      と同一処理〕へ per-chunk フォールバックする。詳細・理由は下記「権威検証を
      行うタイミング」節参照。codex-review P0 指摘への対応——`Err` にすると
      ...
      残りの複製値検査（3 点）を行い、不一致も同様に `Err` にはせず当該 world の
      フォールバックを発動させる（黙って除外もしない。下記「権威検証を行う
      タイミング」節参照）
  ...
  - KNN・`SearchTimeFilter`・`batch_search` はチャンク走査で得たスロットの
    ...
    **権威検証を行うタイミング（確定方針。以下がこのトピックの唯一の定義であり、
    ...
    `Err` から world 単位フォールバックへ置き換えることで両立させる。以下が
    ...
    - **chunked（`layout: chunked`）経路の対応**: チャンク走査で得られるのは
      ...
      存在しない）
    - **フォールバック契約（採用する再設計。本節の中核）**: chunked 経路の
      ポイントルックアップで得た行が、走査元の `(chunk_no, slot)` の正しい
      占有者であることを検査（locator／逆参照の同一性検査）できない場合
      （不一致・ポイントルックアップ自体の miss）、および可視と判定された候補の
      残りの複製値検査（3 点。下記）が不一致となった場合、**いずれも `Err` に
      せず**、そのチャンク（world = `(tenant_id, chunk_no)`）の chunked 高速経路を
      放棄し、**当該 world の権威 `user_rows` 範囲走査
      （`arena.rs::build_filtered_with_rows_and_limits_in_txn` と同一の処理）へ
      per-chunk フォールバックする**。この範囲走査は `user_rows/{table}` の
      `tenant_id` 前置キー範囲全体（同一 tenant の全 `chunk_no` の行を含みうる）を
      対象に実行されるため、走査結果をそのまま採用すると当該 world とは異なる
      `chunk_no`（同 tenant の他 world）の行まで取り込み、重複候補・誤順位・
      件数誤りを招く（codex-review P1 指摘）。**採用する契約（範囲走査結果を
      権威 locator で対象 world に限定する。却下案は下記）**: 上記「行単位
      レイアウトとの互換・移行方針」節の不変条件（`layout: chunked` テーブルは
      常に行フォーマット v3 `[header | locator(chunk_no, slot) | metadata]` の
      みで書き込まれ、`layout: row` テーブルは v2 のまま恒久的に維持され chunked
      高速経路・本フォールバックの対象に一切ならない）を前提に、範囲走査で
      得た各行の v3 header に含まれる `locator` の `chunk_no` が当該 world の
      `chunk_no` と一致するものだけを対象 world の権威結果として採用し、
      `chunk_no` が一致しない行（同 tenant の他 world 由来の行）は本
      フォールバックの対象外として除外する（`layout: chunked` テーブルに
      v2 行が混在することはない不変条件のため、locator を欠く行が対象 world の
      候補として現れることもない）。フォールバック発動時は、当該 world 由来で
      すでにチャンク走査から収集していた候補（複製値検査を通過済みのものを
      含む）を**すべて破棄**し、上記の `chunk_no` 一致でフィルタした範囲走査
      結果で**置換**する（両者を混在させない。`chunk_no` フィルタは走査対象を
      当該 world 分に絞り込むだけであり、`predicate`／`on_visible_row` 判定
      自体は変更しない）。他 world（`chunk_no` が異なる行・別 `tenant_id` の
      チャンク）の候補・処理は当該 world の範囲走査に含まれるか否かに関わらず
      一切変更されない（既に収集済みの他 world 候補は保持されたまま残り、
      破棄・置換は当該 world の候補にのみ適用される）。フォールバック先の
      row-store 範囲走査（`chunk_no` フィルタ適用後）は非再帰・一発限り
      （当該 world について chunked 経路へ再入しない）であり、その走査自体が
      `Err` を返す場合（`user_rows` 側 header デコード失敗等、row-store 経路が
      本来 `Err` にする破損クラス）は、その `Err` をそのまま呼び出し元へ伝播
      する（フォールバックが新たな情報を付加しない）。
      - **「row-store 経路と同一の結果集合になる」という前提（網羅性の根拠と
        ...
        （orphan row）」が生じた場合である**。この孤立行は、(i) どのチャンク
        走査でも live スロットとして列挙されない（chunked 高速経路の候補に
        現れない）ことに加え、(ii) 孤立行自身の `chunk_no` を保持するチャンクが
        それ自体は破損なくデコードでき、かつそのチャンクの他スロットの
        locator／逆参照検査がすべて一致する場合には、素朴には `chunk_no`
        フィルタ済みの置換結果にも現れず短い結果集合を返しうる（クエリ経路
        だけでは検出できない残存リスクとなりうる）。**この残存リスクは、下記
        「孤立行のオンライン検出（world 完全性マーカー）」節で導入する
        `world_completeness` マーカーにより、当該 world のチャンク走査**開始時**
        に検出しフォールバックへ回す**（下記節参照。P1-2 是正——「他スロットが
        整合していればフォールバックが発動せず黙って欠落する」という旧設計の
        欠落を、フォールバック発動そのものをマーカー突合で駆動する設計へ
        置き換えることで解消する）。マーカー突合自体が破損している場合（マーカー
        エントリの欠落・デコード失敗を含む）の扱いは下記節「マーカー自体が
        破損・欠落している場合」を参照。この点検の恒久的な帯域外担保は
        引き続き T8（世界単位の帯域外整合性検証。「5. チャンク単位可視性
        ビットマップ」節）が担い、T8 はオンライン検出（同一 txn 不変条件・
        マーカー突合）が実装バグ等で破れていないかを運用時に確認する役割へ
        位置づけを改める（下記節参照）
      - **却下した代替案（tenant 全体を置換する契約）**: `chunk_no` によるフィルタを
        行わず、フォールバック発動を契機に同 tenant の**全 world**の既収集候補を
        破棄したうえで `tenant_id` 範囲走査結果全体（フィルタなし）を採用する案。
        孤立行の回復可能性を tenant 内の**いずれか**の world のフォールバックへ
        広げられる（`chunk_no` フィルタ案は孤立行自身の home world がフォール
        バックした場合のみ回復するのに対し、tenant 単位案は同 tenant の他の
        world がフォールバックした場合にも巻き込んで回復しうる分だけ回復
        トリガーの範囲が広い）という利点はあったが、下記「孤立行のオンライン
        検出（world 完全性マーカー）」節の採用により孤立行自身の home world の
        フォールバックはもはや「たまたま」ではなく `world_completeness`
        マーカー突合により構造的に駆動される（孤立行が生じれば当該 home world の
        `live_count` がマーカーと一致しなくなり、次回以降のチャンク走査開始時に
        必ずフォールバックする）ため、この利点自体が意味を失っている。この
        限定的な利点に対し、1 チャンクの locator ドリフト・破損という局所的な
        事象が無関係な他 world の chunked 高速経路まで一律に破棄させてしまい
        影響範囲が不必要に拡大するコストの方が上回るため不採用（Rejected）。
        `chunk_no` フィルタ案は影響範囲を当該 world のみに構造的に限定でき、
        「他 world の候補・処理は一切影響を受けない」という本節の主張を
        locator という権威情報のみで機械的に成立させられるため、こちらを
        採用する
      効果:
      1. **フォールバックが発動した world については、結果集合が row-store 経路と
         同一になる**ため、可視行の誤除外・短い結果集合の返却が構造的に起きない
         （Bugbot Medium 指摘への対応。ドリフトで live スロットが無関係な不可視行を
         指す場合も、その world 全体を権威 range 走査へ切り替えるため、正しい
         占有者が検証されずスキップされる余地がない）。**この同一性の保証は
         フォールバックが実際に発動した world についてのみ成立するが、下記
         「孤立行のオンライン検出（world 完全性マーカー）」節の
         `world_completeness` マーカー突合により、孤立行が生じた world は
         次回以降のチャンク走査開始時に必ずフォールバックが発動するため
         （P1-2 是正）、孤立行のケースも実質的にこの保証の対象に含まれる。
         マーカー突合自体が破損している残存ケース（下記「マーカー自体が
         破損・欠落している場合」参照）は T8 の帯域外検証に委ねる**
      ...
      3. **フォールバックの有無・回数は結果集合・エラー応答に現れない**
         ...
         影響しない。ただし、当該 world のコストが chunked 高速経路から
      ...
      2. 各候補スロットについて `user_rows` へポイントルックアップし、
         **metadata はまだデコードせず** `decode_row_tenant_and_visibility` の
         ...
         上記フォールバック契約に従い当該 world を丸ごと row-store 範囲走査へ
         切り替える**（テナントデータに依存しない構造的検査という性質は
         変わらず、`is_visible` 判定より前に行っても他テナントの存在情報を
         漏らさない。ただし帰結が `Err` ではなくフォールバックである点が
         旧来の記述からの変更点）
      ...
      4. **RLS 可視と判定された候補についてのみ**、残りの複製値検査（3 点。
         ...
         せず・黙って除外もせず、上記フォールバック契約に従い当該 world を
         ...
         存在情報を漏らすことはない）
      5. 複製値検査を通過した候補（フォールバックが発動しなかった world）だけが
         ...
         world については、この手順の代わりに row-store 範囲走査結果（同じ
         `predicate` → `on_visible_row` 経路）をそのままアリーナへ積む
    ...
    - **コストへの帰結（両経路で対称。「コスト見積り」節に反映）**: 上記
      ...
      （`O(RLS 可視行数)`。複製値検査と同じ母数）。**フォールバックが発動した
      world については、上記チャンク走査分のコストが無駄になる**（当該 world 分の
      row-store コストに退化するだけではなく、無駄になったチャンク走査・
      ポイントルックアップ分が加算される。上限は「row-store 経路のコスト +
      `O(当該 world のチャンク走査)`」）。フォールバックは発動しても他 world の
      chunked 経路には影響しない
    - **却下した代替案**:
      ...
      - **locator 不一致・複製値検査不一致を即 `Err` にする案（旧設計。不採用）**:
        ...
        （codex-review P0 指摘）。per-chunk フォールバック（上記採用案）は
        `Err` を発生させず結果集合を row-store 経路と同一化することで、
        Bugbot Medium・codex-review P0 双方の要求を同時に満たすため、この案は
        **不採用**とする（Rejected）
    ...
    - **残りの複製値検査の内容（3 点。フォールバック判定。codex-review P0 指摘・
      ...
      **可視と確定した候補**について、チャンク走査で得た `row_ids`（複製）が
      ...
      集合を黙って返す」のではなく、上記フォールバック契約に従い当該 world を
      row-store 範囲走査へ切り替える（結果集合の縮小で改ざん・破損を隠蔽
      しない。fail-closed 性〔誤った行を返さない〕は、フォールバック先が
...
- 行値（`user_rows`）の物理レイアウトは、下記「行単位レイアウトとの互換・移行方針」
  ...
  - **却下した代替案（`user_rows` から embedding を除く案）**: 当初案では
    ...
    当該 world を `user_rows` 範囲走査へフォールバックしても、その範囲走査は
...
- **チャンク単位・スロット単位のいずれも、`slot_bits`／header 集計値による判定は
  ...
  - **意味的不整合**（チャンク側複製と `user_rows` 側権威値の食い違い）: チャンク
    ...
    フォールバック発動）で、`row_ids[slot]`・tenant・`slot_bits` visibility の
    不一致はステップ 3 の `is_visible` 判定を経てステップ 4 で行う残りの複製値
    検査（3 点）で検出・フォールバック発動とする。ただし後者（ステップ 4 の
...
- **クエリ時の突合（3 値照合）**: 上記「権威検証を行うタイミング」節の統一
  ...
  状態を作る）。ある生存行が孤立行になった瞬間（自身の locator が指す
  チャンクの live スロットから到達できなくなった瞬間）、その home world の
  実際の live 集合と `world_completeness`／チャンク header の `live_count` が
  必ず乖離する（同一 txn 不変条件の下では孤立行の発生自体が `live_count` の
  更新漏れと等価であるため）。したがって孤立行は当該 world のチャンク走査
  **開始時**に必ず検出され、row-store 範囲走査へフォールバックする——「他
  スロットが整合していればフォールバックが発動せず黙って欠落する」という
  P1-2 指摘の欠落は生じない
- **マーカー自体が破損・欠落している場合**: `world_completeness` テーブル自体が
  デコードできない、または当該 world のエントリが欠落している場合も、
  **`Err` にはせず**当該 world を row-store 範囲走査へフォールバックする
  （上記と同じ理由——`world_completeness` はチャンク header や `slot_bits` と
...
- **保証範囲の限定（スコープ）**: `world_completeness` が権威として持つのは
  ...
  保証するのは「**生存行の数がチャンク側複製と一致していること（網羅性）**」
  ...
  検証に委ねる）。この限定により、T3b（`slot_bits`／header 集計値による
  スキップ最適化の有効化）を T8 完了前に有効化しない方針（上記「除外判定自体
  の整合性検証（T8）」参照）は変更しない——本マーカーが構造的に保証するのは
...
| `crates/engine/tests/power_loss.rs` | `StorageBackend` 差し替え電源断モデル | 部分書き戻し像でのチャンク値と locator の不整合を検出し、当該 world が `user_rows` 範囲走査へフォールバックすること（`Err` にはならず、結果が row-store 経路と一致すること）の確認。`row_ids[slot]` ⇔ `user_rows` 側 `locator` の相互不整合（再パック中の電源断を含む）に加え、`slot_bits` visibility ⇔ `user_rows` 側 tenant／visibility の不整合（権威検証。上記「権威検証を行うタイミング」節参照）も同一のフォールバック検出対象に含める。`user_rows` 側 header デコード失敗（row-store 経路でも `Err` になる破損）のみ引き続き fail-closed で `Err` とする |
| `crates/engine/tests/index_failure_injection.rs`（RECOVER-9） | 再構築処理の途中失敗注入 | チャンク再パック処理の途中失敗で commit 済み行が無傷であることの確認。途中失敗後も `row_ids` と `locator` が旧配置のまま一貫していること（半端な書き換えが残らないこと）、および `slot_bits` visibility と行側 tenant／visibility の一致が崩れていないことを含める。一致が崩れた状態を注入した場合に、当該 world がフォールバックし結果が row-store 経路と一致すること（`Err` にならないこと）を検証対象に加える |
...
- **読み取り（フィルタなし経路。`WHERE`・宣言的フィルタ API を伴わない KNN・
  ...
  上限見積りとして提示するに留める）。**フォールバックが発動する world がある
  場合**、上記の見積りに加え、当該 world 分の無駄になったチャンク走査・
  ポイントルックアップ・locator 検査のコストと、`user_rows` 範囲走査
  （row-store 経路と同一コスト）が加算される（コストは「row-store 経路 +
  `O(当該 world のチャンク走査)`」が上限であり、フォールバックが chunked 経路の
  コストを完全に相殺することはない）。フォールバックの発生頻度・レイテンシへの
...
| T8 | 世界単位（`(tenant_id, chunk_no)` 単位）整合性検証ツール（`live_count`／`public_live`／`private_live`／`slot_bits` と対応する `user_rows` 全件走査結果の突合。運用者専用・クエリ応答経路には結線しない。「5. チャンク単位可視性ビットマップ」節「除外判定自体の整合性検証」参照。「2. 削除（tombstone）」節「集計値の原子更新不変条件」の恒久的な帯域外担保を兼ねる）。**孤立行の検出・回復自体はステップ 0（`world_completeness` 突合）によりオンラインで担保されるため（P1-2 是正。上記「孤立行のオンライン検出（world 完全性マーカー）」節参照）、T8 の役割は「オンライン検出そのものが正しく機能しているか」の帯域外バックストップ（`world_completeness` エントリ自体の破損・欠落・`generation`／`live_count` の実測値からのドリフトの検出）と、オンライン検出の保証範囲外である per-slot visibility ドリフト（`public_live`／`private_live`／`slot_bits` visibility ビット）の網羅性検証に位置づけを改める** | T2 | `examples/crash_tool*.rs` 系（新規サブコマンド）・`storage/chunk.rs` |
...
| テナント境界（P0） | — | 可視性判定は `PolicyContext::is_visible` の単一照合パスのみを用いる。チャンクスキップは同 predicate の結果から導出し、独自テナント比較を新設しない。チャンクはテナント単位キー。エラーに他テナントの存在情報を含めない。**チャンク側 `slot_bits`／header 集計値・`world_completeness` エントリはいずれも `user_rows` 側行ヘッダの tenant／visibility から派生した複製であり単一権威ではない**（`user_rows` 側行ヘッダが権威。`embedding` 本体も P1-1 是正により `user_rows` 側が権威コピーで `user_vecs` チャンク側は走査高速化用の派生複製である）。単一権威原則は方向で保証範囲が異なる: **admission 方向は無条件**——チャンク走査を開始する前に、まず `world_completeness`（権威 `(generation, live_count)`）とチャンク header の `live_count` を突合し（ステップ 0）、チャンク走査中は `slot_bits` から求めた `popcount` とも 3 値照合する（ステップ 1）。いずれか 2 値でも不一致なら `Err` にはせず当該 world を丸ごとフォールバックする——これにより「孤立行（生存行が到達不能になり黙って結果集合から欠落する事象）」はクエリ時にオンラインで検出される（P1-2 是正。詳細は「5. チャンク単位可視性ビットマップ」節「孤立行のオンライン検出（world 完全性マーカー）」参照）。この点検を通過した world についてのみ、チャンク走査で候補になりうる全 live スロットについて、まず `user_rows` へポイントルックアップし header デコードのみで権威 tenant／visibility を取得する。取得直後、テナントデータに一切依存しない構造的整合性検査として、取得した行の header が走査元の `(chunk_no, slot)` を指し戻すこと（locator／逆参照の同一性検査）を確認し、不一致・ポイントルックアップ自体の miss・チャンク header 自体のデコード失敗は、いずれも**`Err` にはせず**、そのチャンク（world = `(tenant_id, chunk_no)`）の chunked 高速経路を放棄し、**当該 world の権威 `user_rows` 範囲走査（`arena.rs::build_filtered_with_rows_and_limits_in_txn` と同一処理）へ per-chunk フォールバックする**（フォールバック発動時は当該 world 由来の既収集候補を破棄し、`user_rows` の当該 `tenant_id` 範囲走査結果のうち v3 header `locator` の `chunk_no` が当該 world と一致する行のみで置換する〔`chunk_no` フィルタにより同 tenant の他 world の行を取り込まない。詳細・却下案は上記「権威検証を行うタイミング」節「フォールバック契約」参照〕。他 world の候補は影響を受けない。codex-review P0 指摘・P1 指摘への対応——`Err` にすると他テナント側の locator 状態でクエリの成否が変わるエラーオラクルになるため）。フォールバック先の row-store 走査自体が `Err` を返す場合（`user_rows` 側 header デコード失敗等、row-store 経路でも `Err` になる破損）は、その `Err` をそのまま伝播する。この検査を通過した候補についてのみ `PolicyContext::is_visible` による RLS 判定を行う。**RLS 不可視と判定された候補はここで残りの複製値検査（3 点）を行わずフォールバックも発動させず、黙ってスキップする**（他テナントの不可視行の `slot_bits`・可視性集計のドリフトを観測させないため。Issue #137 と同種のエラーオラクルを避ける設計であり、row-store〔`arena.rs::build_filtered_with_rows_and_limits_in_txn`〕が header デコード成功後に不可視行を検証なしでスキップする挙動と同一の契約。Bugbot High 指摘への対応）。**RLS 可視と判定された候補についてのみ**、`WHERE` 経路・フィルタなし経路のいずれも距離計算・ランキングより前（`WHERE` 系はさらに metadata デコードより前）に残りの複製値検査（`row_ids[slot]`・tenant・`slot_bits` visibility の 3 点突合。locator の同一性は上記のとおり `is_visible` 判定より前で全候補に対し検査済みのため、ここでは扱わない。`layout: chunked` に限って発生するため常に 3 点とも検証する）を行い、不一致も同様に `Err` にせず・黙って除外もせず、当該 world を row-store 範囲走査へフォールバックする（誤った行を返さない fail-closed 性は、フォールバック先が権威経路であることで担保される。詳細な順序は「権威検証を行うタイミング」節参照）。**除外方向（`slot_bits`／header 集計値によるチャンク・スロットスキップ）は保証が限定的**——ポイントルックアップに到達する前の除外はこの複製値検査の対象外であり、コミット後の複製値のみの破損による誤除外はチャンク走査だけでは検出できない残存リスクとして明示する（ポイントルックアップに到達した候補はフォールバックで回復するため、残存リスクは「候補にすら到達しない」除外方向の最適化に限定される）。**`world_completeness` マーカーが構造的に保証するのは生存行数（`live_count`）の一致（＝孤立行が無いこと）のみであり、`public_live`／`private_live`／`slot_bits` visibility ビットの個別の正しさまでは保証しない**（上記「孤立行のオンライン検出」節「保証範囲の限定（スコープ）」参照）ため、この残存リスクは世界単位の帯域外整合性検証（T8）を前提として運用時に担保し、**T8 実装まで除外方向の最適化を有効化しない**（「権威検証を行うタイミング」節・「5. チャンク単位可視性ビットマップ」節参照）。**フィルタなし経路（`WHERE`・宣言的フィルタ API を伴わない KNN・`SearchTimeFilter`・`batch_search`）も、上記の統一順序（header デコード→locator／逆参照の同一性検査→ `is_visible` 判定→不可視行は検証せず黙ってスキップ→可視行のみ残りの複製値検査）でランキング前の候補段階に権威検証を行う（ランキング後に不足分を再取得する案は、非権威な複製値でランキングしてから検証する構造ゆえ存在情報漏えいの残存差分があり Rejected として却下済み。locator 不一致・複製値検査不一致を即 `Err` にする旧案も、他テナント側の保存状態でクエリの成否が変わるエラーオラクルになるため Rejected として却下済み）ことで「短い結果集合を返さない」「不可視行が順位・件数に影響しない」「複製ドリフトが `Err` として観測されない」「`Err` は row-store 経路でも `Err` になる破損に限られる」の 4 不変条件を満たすことを実装タスク（T3a・T5）の検証対象とする（フォールバックの有無・回数は結果集合・エラー応答には現れず〔呼び出し元の wire 可視契約には影響しない〕、レイテンシ差のみが残存としてプロセス内メトリクス／T8 の帯域外検証の対象となる。Bugbot Medium 指摘・codex-review P0 指摘への対応。「権威検証を行うタイミング」節参照） |
