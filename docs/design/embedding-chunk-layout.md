    ...
    **権威検証を行うタイミング（確定方針。以下がこのトピックの唯一の定義であり、
    ...
    - **フォールバック契約（採用する再設計。本節の中核）**: chunked 経路の
      ...
      - **「row-store 経路と同一の結果集合になる」という前提（網羅性の根拠と
        ...
        locator／逆参照検査がすべて一致する場合には、その world のフォール
        バックも発動しないため、`chunk_no` フィルタ済みの置換結果にも現れず
        **短い結果集合を返す残存リスクとなる**（本節冒頭の「両 world を混在
        させない」設計判断そのものは変わらず、この孤立行検出はいずれの
        フォールバック粒度〔本節が採用する world 単位・却下した tenant 単位の
        いずれ〕でも「フォールバックが実際に発動しない限り検出できない」という
        構造は共通する。ただし回復のトリガー範囲は両者で異なる——tenant 単位の
        全走査では同 tenant の**いずれか**の world がフォールバックすれば
        孤立行も巻き込んで回復しうるのに対し、`chunk_no` フィルタ済みの world
        単位フォールバックでは**孤立行自身の home world** がフォールバック
        しない限り回復しない、という差分は下記「却下した代替案」節に記載する。
        いずれの粒度でも網羅性を保証しない点は同じであり、帯域外の検証が
        必要になることに変わりはない）。したがって孤立行の検出・回復は T8（世界単位の
        帯域外整合性検証。上記「5. チャンク単位可視性ビットマップ」節）の
        スコープに、チャンク header／複製値の整合性検証だけでなく「**`layout:
        chunked` テーブルの `user_rows` 側の全生存行が、自身の locator が指す
        チャンクの live スロットから逆参照で到達できること（網羅性検証）**」
        を追加し、運用時の完全性担保として明示する（T8 完了までは、この
        孤立行クラスの残存リスクは他の複製ドリフト残存リスクと同様に許容
        する。「除外方向」の限定的保証と同種の性質であり、新たなクラスの
        リスクを本節の設計判断が追加するものではない）
      - **却下した代替案（tenant 全体を置換する契約）**: `chunk_no` によるフィルタを
        ...
        トリガーの範囲が広い）という利点はあるが、それでも回復は「たまたま
        いずれかの world がフォールバックする」ことに依存する偶発的なもので
        あり網羅性を**保証**しない点は `chunk_no` フィルタ案と同じである（結局
        どちらの粒度でも帯域外の網羅性検証〔T8〕が本質的に必要になる）。この
      ...
      1. **フォールバックが発動した world については、結果集合が row-store 経路と
         ...
         占有者が検証されずスキップされる余地がない）。**ただし、この同一性の
         保証はフォールバックが実際に発動した world に限られる**——上記
         「「row-store 経路と同一の結果集合になる」という前提」節に記載のとおり、
         孤立行（自身の locator が指すチャンクが破損なくデコードでき、そのチャンクの
         他スロットも整合しているため世界単位のフォールバックが発動しないケース）は
         この保証の対象外であり、T8 の網羅性検証に委ねる残存リスクである
