  ...
  - **`slot_bits` の visibility ビット・header の `public_live`／`private_live`
    ...
    **等の書き込み経路が同一 txn 内で複製した派生値であり、権威（authoritative）
    ...
    - **admission 方向（無条件に保証）**: チャンク側複製の値だけを根拠に行を
      ...
      構造的検査。不一致・ポイントルックアップ自体の miss はいずれも占有者を
      特定できない「ストレージ破損」クラスとして他テナント行であっても常に
      `Err`。`arena.rs` の header デコード失敗時の扱いと同一の契約）を経てから、
      `user_rows` 側権威 tenant／visibility で RLS 判定
      （`PolicyContext::is_visible`）を行い、この判定こそが admission の唯一の
      根拠である。RLS 不可視と判定された候補は残りの複製値検査（3 点。
      `row_ids[slot]`・tenant・`slot_bits` 可視性。locator の同一性検査は
      それより前に済んでいる）を経ずに黙ってスキップし `Err` にはしない
      （他テナントの不可視行の複製ドリフト〔`slot_bits`・可視性集計〕を `Err`
      経由で観測させない。Issue #137 と同種のエラーオラクルを避ける設計であり、
      row-store（`arena.rs::build_filtered_with_rows_and_limits_in_txn`）が
      header デコード成功後に不可視行を検証なしでスキップする挙動と同一の契約。
      Bugbot High 指摘への対応）。RLS 可視と判定された候補についてのみ続けて
      残りの複製値検査（3 点）を行い、不一致は `Err`（黙って除外しない）
  ...
  - KNN・`SearchTimeFilter`・`batch_search` はチャンク走査で得たスロットの
    ...
    **権威検証を行うタイミング（確定方針。以下がこのトピックの唯一の定義であり、
    ...
    - **統一順序（単一のルール。段階〔候補時点／返却時点〕による例外を設けない。
      ...
      あった（Bugbot High 指摘・初回対応）。さらにその後の改訂では、4 点検証を
      `is_visible` 判定より前に置いていたため、可視性未確定の全 live スロット
      （他テナントの private 行を含む）に対して 4 点検証が実行され、そこでの
      複製ドリフトが `Err` として観測可能になっていた。これは
      `arena.rs::build_filtered_with_rows_and_limits_in_txn` が「header デコード
      成功後は不可視行を検証せずスキップする」ことで避けているエラーオラクル
      （他テナント行の存在・破損を `Err` 経由で観測できる。Issue #137 と同種）を
      再導入するものであり、`is_visible` 判定を 4 点検証より前に置く順序へ
      再改訂した（Bugbot High 指摘・再対応）。ただしこの再改訂は「4 点検証
      全体」を `is_visible` より後ろへ動かしただけであり、`is_visible` 判定
      そのものが参照する tenant／visibility が、`row_ids[slot]` の複製ドリフト
      によって**そもそも誤ったスロット占有者の行から取得されうる**欠落が
      残っていた。ポイントルックアップした行が本当に走査元の
      `(chunk_no, slot)` の占有者であるかを確認しないまま `is_visible` を
      評価すると、ドリフトで live スロットが（無関係な）不可視行を指す場合に
      その不可視行の tenant／visibility を根拠に `is_visible` が偽と判定して
      しまい、本来可視であるはずの正しい占有者ごとスロットが検証されず
      スキップされる（Bugbot Medium 指摘）。これは他テナント行の複製値
      ドリフト（`slot_bits`・可視性集計）とは性質が異なり、**スロットの占有者
      自体を特定できていない**ために可視性判定の前提が成立しない状態であり、
      `arena.rs::build_filtered_with_rows_and_limits_in_txn` の header デコード
      失敗時の扱い（ヘッダ部自体が壊れていて可視性を判定できない場合は
      「不可視だからスキップ」と判断できないため fail-closed に伝播する。
      `decode_row_tenant_and_visibility` 呼び出し直後のコメント参照）と同じ
      「ストレージ破損」クラスに属する。したがって、`user_rows` へのポイント
      ルックアップで取得した行の header が走査元の `(chunk_no, slot)` を
      指し戻すことを確認する **locator／逆参照の同一性検査**（テナントデータに
      一切依存しない構造的整合性検査）を、header デコード直後・`is_visible`
      判定より**前**に置く。この検査（ポイントルックアップ自体が
      `row_ids[slot]` の指す `id` を見つけられない miss を含む）に失敗した
      場合は、可視性の判定材料が得られないため他テナント行であっても常に
      `Err` とする（header デコード失敗と同一の破損クラス。可視性ドリフトの
      黙殺スキップとは別物であり新たなエラーオラクルを追加しない。Bugbot
      Medium 指摘・再々対応）:
      ...
      2. 各候補スロットについて `user_rows` へポイントルックアップし、
         **metadata はまだデコードせず** `decode_row_tenant_and_visibility` の
         ...
         `is_visible` 判定を行うことができないため、`user_rows` 側の tenant が
         何であっても——他テナント行が原因であっても——検証せずに
         `Err`（ストレージ破損クラス。header デコード失敗と同じ扱い）を返す。
         この検査はテナントデータ（tenant／visibility）を一切参照しない構造的
         検査であり、可視・不可視の確定より前に行っても他テナントの存在情報を
         漏らさない
      3. ステップ 2 の locator／逆参照の同一性検査を通過した候補についてのみ、
         ...
         3 点。以下「複製値検査」）を行わず・`Err` にもせず、ここで黙って
         スキップする**（`continue` に相当。row-store の `arena.rs` が
         「header デコードが成功した不可視行は検証せずスキップする」挙動と
         同一の契約。他テナントの不可視行に対して複製ドリフト（`slot_bits`・
         可視性集計）を検証すること自体が、その行の存在・破損を `Err` 経由で
         呼び出し元へ観測させるエラーオラクルになるため、可視性が確定する前の
         複製値検査は行わない。ただしステップ 2 の locator／逆参照の同一性検査
         はテナントデータに依存しない構造的検査のため、この黙殺スキップの
         対象外——不可視候補であっても占有者を特定できなければステップ 2 の
         時点で既に `Err` になっている）
      4. **RLS 可視と判定された候補についてのみ**、残りの複製値検査（3 点。
         ...
         済みのためここでは扱わない）を行う。1 つでも不一致なら可視・不可視の
         どちらとも確定せず、**黙って除外もせず** fail-closed で `Err` を返す
         （改ざん・破損を結果集合の縮小で隠蔽しない。この `Err` は可視と
         確定済みの候補についてのみ発生するため、他テナントの存在情報を
         漏らさない）
      5. 複製値検査を通過した候補だけが次段へ進む。`WHERE`・宣言的フィルタ API
         ...
         `on_visible_row` に必要な metadata デコードを複製値検査の**後**に
         結線する）。伴わない経路（フィルタなし KNN・`SearchTimeFilter`・
    ...
    - **コストへの帰結（両経路で対称。「コスト見積り」節に反映）**: 上記
      ...
      `O(候補 live スロット数)` のまま増えない（新たな `O()` 項の追加ではなく、
      既存のポイントルックアップ段に検査を 1 つ足すのみ）。ステップ 3 の
      `is_visible` 判定はステップ 2 で得た権威値に対して行うため追加の
      ポイントルックアップを伴わない。ステップ 4 の複製値検査（3 点）は、
      ステップ 3 で RLS 可視と判定された候補分のみに発生する
      （`O(RLS 可視行数)`。不可視候補は検証を経ずにステップ 3 で
      スキップされるため、複製値検査の母数から外れる。以前の「4 点検証は
      `O(RLS 可視行数)`」という見積りのうち、locator の同一性検査分は
      `O(候補 live スロット数)` へ切り出し、残り 3 点のみが `O(RLS 可視行数)`
      に留まる）。ステップ 5 の metadata デコードは `WHERE`・宣言的フィルタ
      API を伴う経路でのみ、ステップ 3 で RLS 可視と判定された行数分だけ
      追加発生する（`O(RLS 可視行数)`。複製値検査と同じ母数）。フィルタなし
      経路はステップ 5 の metadata デコード分のコストを払わないが、ステップ 2
      のポイントルックアップ・locator 同一性検査・ステップ 4 の複製値検査は
      いずれも `WHERE` 付き経路と同じ母数（順に `O(候補 live スロット数)`・
      `O(候補 live スロット数)`・`O(RLS 可視行数)`）を払う。「フィルタなし
      経路だけが返却行数分のポイントルックアップで済む」という以前の見積りは
      撤回する
    ...
    - **不変条件（T3a・T5 の検証対象に追加。Bugbot High 指摘・Bugbot Medium
      指摘への対応）**:
      1. **短い結果集合を返さない**: 可視行が k 件以上存在するにもかかわらず、
         不可視行の混入・除外に起因して k 未満の結果が返ることはない（上記
         統一順序ではステップ 2 の locator／逆参照の同一性検査で占有者を
         誤って取り違えることなく正しいスロット占有者の tenant／visibility を
         得たうえでステップ 3 の `is_visible` 判定を行うため、`row_ids[slot]`
         の複製ドリフトで live スロットが無関係な不可視行を指していても、
         その誤同定によって本来可視な占有者が検証されずスキップされることは
         ない〔占有者を特定できない場合は `Err` とし、黙ってスキップしない。
         Bugbot Medium 指摘への対応〕。ステップ 3 の時点で既に可視行のみに
         絞られ、後続のステップ〔複製値検査・metadata デコード・アリーナ
         構築〕へ不可視行が混入しないことと合わせて構造的に満たす）
      2. **不可視行が順位・件数に影響しない（存在情報漏えいなし）**: 他テナントの
         private 行を含む不可視行の embedding・存在が、返却される可視行の順位・
         件数・有無を左右しない（上記統一順序はランキング候補に不可視行を
         含めないため構造的に満たす）
      3. **不可視行の複製値ドリフトが `Err`（エラーオラクル）として観測され
         ない。locator の破損は現行 row-store と同一の「ストレージ破損」
         クラスとして扱う**: 他テナントの不可視行について `slot_bits`・
         可視性集計の複製値のみがドリフト・破損していても、それを理由に
         呼び出し元へ `Err` が返ることはない（上記統一順序はステップ 3 で
         不可視行を残りの複製値検査〔3 点〕の対象前にスキップするため、構造的に
         満たす。Issue #137 と同種のエラーオラクル再導入を防ぐための条件。
         Bugbot High 指摘への再対応）。一方、`user_rows` へのポイント
         ルックアップで得た行の header が走査元の `(chunk_no, slot)` を
         指し戻さない場合（locator／逆参照の同一性検査の不一致・ポイント
         ルックアップ自体の miss）は、可視性の判定材料自体が得られない
         「ストレージ破損」であり、`arena.rs::build_filtered_with_rows_and_
         limits_in_txn` の header デコード失敗が他テナント行であっても常に
         `Err` へ伝播する現行契約と同一クラスとして、テナントを問わず
         `Err` とする（`is_visible` 判定より前に位置するため「不可視行だから
         検証対象外」という除外の適用対象にならない。Bugbot Medium 指摘への
         対応）
      3 条件は `crates/engine/tests/index_failure_injection.rs`（RECOVER-9）・
    ...
    - **`slot_bits`／header 集計値によるスキップ最適化（T3b）は検査コストを
      ...
      ポイントルックアップ数）のみである
    - **locator／逆参照の同一性検査（ステップ 2。fail-closed。Bugbot Medium
      指摘への対応）**: `user_rows` へのポイントルックアップで取得した行の
      header に含まれる `locator(chunk_no, slot)`（行フォーマット v3）が、
      走査元の `(chunk_no, slot)` と一致することを検査する。この検査は
      テナントデータ（tenant／visibility）を一切参照しない構造的整合性検査
      であり、`is_visible` 判定（ステップ 3）より前に置いても他テナントの
      存在情報を漏らさない。不一致・ポイントルックアップ自体の miss
      （`row_ids[slot]` の指す `id` が存在しない場合）は、いずれもスロットの
      占有者を特定できていない状態であり可視性を判定する前提が成立しないため、
      `user_rows` 側の tenant が何であっても検証せずに `Err` とする
      （`arena.rs::build_filtered_with_rows_and_limits_in_txn` の header
      デコード失敗時の扱いと同一の「ストレージ破損」クラス。可視性が未確定の
      候補を黙ってスキップする経路〔ステップ 3〕とは異なり、新たなエラー
      オラクルを追加しない）。この検査は候補になりうる全 live スロットに対して
      発生する（`O(候補 live スロット数)`。ステップ 2 の header デコードと
      同じ母数・同じデータから追加のポイントルックアップなしに行える）
    - **残りの複製値検査の内容（3 点。fail-closed。codex-review P0 指摘・
      Bugbot High 指摘・Bugbot Medium 指摘への対応）**: この経路（チャンク
      走査から `row_ids[slot]` を経由した点検索）は `layout: chunked`（v3）の
      テーブルに限って発生する（`layout: row` のテーブルは `user_vecs` を
      持たず本節の対象外。上記「物理設計案」節参照）ため、次の 3 点は常に
      すべて揃って検証できる（`locator(chunk_no, slot)` の一致は上記の
      locator／逆参照の同一性検査としてステップ 2 で先に検査済みのため、
      ここでは扱わない）:
      ...
      **可視と確定した候補**について、チャンク走査で得た `row_ids`（複製）が
      ...
      集合を黙って返す」のではなく fail-closed エラーとして伝播させる。ただし、
      この検証は **RLS 可視と判定された候補についてのみ**保証するものであり、
      次の 2 種類の候補はいずれも対象外である:
      - **不可視と判定された候補**（統一順序ステップ 3。locator／逆参照の
        ...
        （`slot_bits`・可視性集計）を `Err` として観測可能にしない
        （エラーオラクル防止。Issue #137 と同種。Bugbot High 指摘への対応）
        ための意図的な除外であり、admission に関わる安全性上の要請による
...
- **チャンク単位・スロット単位のいずれも、`slot_bits`／header 集計値による判定は
  ...
  - **意味的不整合**（チャンク側複製と `user_rows` 側権威値の食い違い）: チャンク
    ...
    ステップ 2 の locator／逆参照の同一性検査（`is_visible` 判定より前・テナントを
    問わず `Err`）で、`row_ids[slot]`・tenant・`slot_bits` visibility の不一致は
    ステップ 3 の `is_visible` 判定を経てステップ 4 で行う残りの複製値検査（3 点）
    で検出・`Err` とする。ただし後者（ステップ 4 の複製値検査）の検出は **RLS
    可視と判定された候補**（残りの複製値検査節参照）にのみ保証される。次の
    2 種類の候補はいずれもステップ 4 の複製値検査の対象に含まれない
    （locator／逆参照の同一性検査はステップ 2 でテナントを問わず行うため、この
    2 種類の限定の対象外）:
    ...
    ドリフトをクエリ経路の `Err` として観測させない（Issue #137 と同種の
  ...
  - **除外判定自体の整合性検証（T8。上記の残存リスクへの対応）**: 除外方向の
    ...
    **T8 が実装されるまでは、`layout: chunked` テーブルのチャンク header／
    `slot_bits` によるスキップ最適化（本節・「残りの複製値検査の内容」節の対象を
    減らす方向の最適化）を有効化しない**。これにより「不一致を黙って除外しない」契約は、
...
- **読み取り（フィルタなし経路。`WHERE`・宣言的フィルタ API を伴わない KNN・
  ...
  削減される計算）。一方、権威検証の統一順序（ステップ 2・3）では、チャンク
  走査で候補になりうる全 live スロットに対し **header デコードのみ**のポイント
  ルックアップと、同じ header デコード結果に対する locator／逆参照の同一性
  検査が発生するため（`slot_bits` スキップが T3b で有効化されるまでは概ね
  走査対象の可視候補全体）、`O(候補 live スロット数)` 項を持つ（metadata の
  デコードは発生しないため、下記「metadata 必須経路」の `O(RLS 可視行数)` 項
  より係数は小さいが、母数〔候補 live スロット数〕自体は縮小しない。locator／
  逆参照の同一性検査は追加のポイントルックアップを伴わないため `O()` の項を
  増やさない。「この経路のポイントルックアップは最終的な返却行数分のみに
  限られる」という以前の見積りは撤回する）。ステップ 3 の `is_visible` 判定で
  不可視と確定した候補はここで検証なしにスキップされるため、続くステップ 4 の
  残りの複製値検査（3 点）は RLS 可視と判定された候補分のみに発生する
  （`O(RLS 可視行数)`）。この経路は metadata デコード（ステップ 5）を行わない
  ため、フィルタなし経路の総コストは `O(N_scan / N) 償却分 + O(候補 live
  スロット数)`（header デコード＋locator／逆参照の同一性検査）
  `+ O(RLS 可視行数)`（残りの複製値検査）の和になる。ランキング後に不足分を再取得する案は、非権威な複製値で
...
- **読み取り（metadata 必須経路。`WHERE`・宣言的フィルタ API を伴うクエリ）**:
  ...
  行数分だけ発生する。読み取りコストのモデルは次の 4 項の和で表す:
...
| T3a | 読み取り経路の結線（`VectorArena`・`SearchTimeFilter`・`batch_search` のチャンク走査＋権威検証〔「権威検証を行うタイミング」節の統一順序（header デコード→locator／逆参照の同一性検査（テナントを問わず不一致は `Err`）→ `is_visible` 判定→不可視行は検証せず黙ってスキップ→可視行のみ残りの複製値検査（3 点）→ metadata／SCALAR）。フィルタなし経路も同節の統一順序で実装する〕。集計は下記「T3a の対象範囲・集計の読み取り元」のとおりチャンク走査へ移さない。チャンク header／`slot_bits` によるスキップ最適化はコード上実装するが**既定無効**（フィーチャーフラグ／設定で off）とし、T3b（T8 完了後）まで有効化しない。検証には実際に書き込まれたチャンク〔T2〕と `layout: chunked` 選択・v2/v3 分岐〔T4〕の両方が必要なため依存に含める。**検証対象に「権威検証を行うタイミング」節の不変条件（短い結果集合を返さない・不可視行が順位/件数に影響しない・不可視行の複製値ドリフトが `Err` として観測されない〔locator 破損は現行と同一の破損クラスとして常に `Err`〕）を追加**〔Bugbot High 指摘・Bugbot Medium 指摘への対応。他テナントの不可視行・private 行を混在させたコーパスで、可視行が k 件以上あるとき返却が k 未満へ縮まらないこと、不可視行の有無で可視行の順位・件数が変化しないこと、他テナントの不可視行の `slot_bits`・可視性集計のドリフトが `Err` として観測されないこと、および `row_ids[slot]` がドリフトして無関係な不可視行を指す場合にスロットが黙って落ちず `Err` になること（locator／逆参照の同一性検査の直接回帰）を回帰テスト化する〕） | T1・T2・T4 | `arena.rs`・`rls.rs`・`batch_search.rs`・`sql/aggregate.rs`・`sql/group_by.rs` |
...
| T5 | 回復検証の拡張（crash_tool サブコマンド・cross-table 3 テーブル・power_loss・RECOVER-9 注入。「2. 削除（tombstone）」節「集計値の原子更新不変条件」の途中失敗検証を含む。**T3a と同じ「短い結果集合を返さない」「不可視行が順位/件数に影響しない」「不可視行の複製値ドリフトが `Err` として観測されない（locator 破損は現行と同一の破損クラスとして常に `Err`）」不変条件を、途中失敗・再構築後の状態でも保つことを RECOVER-9 注入の検証対象へ追加**〔Bugbot High 指摘・Bugbot Medium 指摘への対応〕） | T2 | `scripts/`・`examples/crash_tool*.rs`・`tests/` |
...
| テナント境界（P0） | — | 可視性判定は `PolicyContext::is_visible` の単一照合パスのみを用いる。チャンクスキップは同 predicate の結果から導出し、独自テナント比較を新設しない。チャンクはテナント単位キー。header 破損は fail-closed（スキップしない）。エラーに他テナントの存在情報を含めない。**チャンク側 `slot_bits`／header 集計値は `user_rows` 側 tenant／visibility の派生複製であり単一権威ではない**（`user_rows` 側行ヘッダが権威）。単一権威原則は方向で保証範囲が異なる: **admission 方向は無条件**——チャンク走査で候補になりうる全 live スロットについて、まず `user_rows` へポイントルックアップし header デコードのみで権威 tenant／visibility を取得する。取得直後、テナントデータに一切依存しない構造的整合性検査として、取得した行の header が走査元の `(chunk_no, slot)` を指し戻すこと（locator／逆参照の同一性検査）を確認し、不一致・ポイントルックアップ自体の miss はスロットの占有者を特定できていない「ストレージ破損」として、`user_rows` 側の tenant が何であっても常に `Err` とする（`arena.rs::build_filtered_with_rows_and_limits_in_txn` の header デコード失敗時の扱いと同一クラス。可視性が未確定な段階で行う検査のため新たなエラーオラクルにはならない）。この検査を通過した候補についてのみ `PolicyContext::is_visible` による RLS 判定を行う。**RLS 不可視と判定された候補はここで残りの複製値検査（3 点）を行わず黙ってスキップし `Err` にはしない**（他テナントの不可視行の `slot_bits`・可視性集計のドリフトを `Err` 経由で観測させないため。Issue #137 と同種のエラーオラクルを避ける設計であり、row-store〔`arena.rs::build_filtered_with_rows_and_limits_in_txn`〕が header デコード成功後に不可視行を検証なしでスキップする挙動と同一の契約。Bugbot High 指摘への対応）。**RLS 可視と判定された候補についてのみ**、`WHERE` 経路・フィルタなし経路のいずれも距離計算・ランキングより前（`WHERE` 系はさらに metadata デコードより前）に残りの複製値検査（`row_ids[slot]`・tenant・`slot_bits` visibility の 3 点突合。locator の同一性は上記のとおり `is_visible` 判定より前で全候補に対し検査済みのため、ここでは扱わない。`layout: chunked` に限って発生するため常に 3 点とも検証する）を行い、不一致は黙って除外・再判定せず `Err` とする（詳細な順序は「権威検証を行うタイミング」節参照）。**除外方向（`slot_bits`／header 集計値によるチャンク・スロットスキップ）は保証が限定的**——ポイントルックアップに到達する前の除外はこの複製値検査の対象外であり、コミット後の複製値のみの破損による誤除外はクエリ経路では検出できない残存リスクとして明示する。この残存リスクは世界単位の帯域外整合性検証（T8）を前提として運用時に担保し、**T8 実装まで除外方向の最適化を有効化しない**（「権威検証を行うタイミング」節・「5. チャンク単位可視性ビットマップ」節参照）。**フィルタなし経路（`WHERE`・宣言的フィルタ API を伴わない KNN・`SearchTimeFilter`・`batch_search`）も、上記の統一順序（header デコード→locator／逆参照の同一性検査→ `is_visible` 判定→不可視行は検証せず黙ってスキップ→可視行のみ残りの複製値検査）でランキング前の候補段階に権威検証を行う（ランキング後に不足分を再取得する案は、非権威な複製値でランキングしてから検証する構造ゆえ存在情報漏えいの残存差分があり Rejected として却下済み）ことで「短い結果集合を返さない」「不可視行が順位・件数に影響しない」「不可視行の複製値ドリフトが `Err` として観測されない（locator 破損は現行と同一の破損クラスとして常に `Err`）」の 3 不変条件を満たすことを実装タスク（T3a・T5）の検証対象とする（Bugbot High 指摘・Bugbot Medium 指摘・codex-review P0/P1 指摘への対応。「権威検証を行うタイミング」節参照） |
