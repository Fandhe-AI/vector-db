...
- 新規動的テーブル `user_vecs/{table_name}`（`user_rows/{table}` と同じ命名規約・
  ...
  - KNN・`SearchTimeFilter`・`batch_search` はチャンク走査で得たスロットの
    ...
    **権威検証を行うタイミングは「RLS predicate 通過後・ランキング前の候補
    段階」であり、WHERE・宣言的フィルタ API 経路もフィルタなし経路も同じ
    タイミングに一意に決まる（「返却対象へ絞ってから検証する」方式は不可視行
    が順位・件数へ影響するため採用しない。後述の案 (a) と同一の確定方針）**
    ...
    - KNN（`WHERE`・宣言的フィルタ API を伴わない場合）・`SearchTimeFilter`・
      `batch_search`（RLS のみを適用する経路。**Bugbot High 指摘への対応・是正版**）:
      「ランキング後に返却対象へ絞ってから権威検証」という以前の記述は、現行の
      行ストア brute-force 経路（`arena.rs::build_filtered_with_rows_and_limits_in_txn`
      が `predicate`〔RLS 段〕を距離計算・ランキングより**前**に適用し、不可視行を
      そもそもアリーナへ確保しない固定順序。冒頭「現状の機構」表 #3 参照）と
      矛盾していた。加えてチャンク走査では `slot_bits` によるスキップ最適化が
      T3b（T8 完了）まで既定無効のため、ランキング段の候補プールに他テナントの
      private 行を含む不可視スロットがそのまま混入する。この結果、上位 k 件を
      可視行が占めるべきところを不可視行が占め、返却直前の権威検証で不可視行が
      落ちた分だけ k 未満の短い結果集合が黙って返る欠陥があった。是正として
      **ランキング前の候補段階での権威検証（(a) 案）に一本化する**（いずれも
      下記「不変条件」を満たすことを実装タスクの検証対象とする）:
      - **(a) ランキング前の候補段階での権威検証（採用・確定）**: チャンク走査で
        得た各 live スロットについて、距離計算・ランキングへ candidate として
        積む**前**に上記 5 ステップ（4 点検証込み。metadata のデコードは不要な
        ため tenant／visibility 判定のみを用い、step 4 の metadata デコードは
        省略できる）を実行し、admission が確定した行だけを距離計算・ランキング
        対象とする。現行の行ストア brute-force 経路（RLS predicate → アリーナ
        構築 → 距離計算・ランキング）と同じ「RLS 通過後・ランキング前」の順序に
        一致させ、`WHERE` 付き経路との構造も揃う。コスト面では、この経路も
        「返却行数分」ではなく「チャンク走査で候補になりうる全 live スロット数」
        分のポイントルックアップが RLS 段として発生する（`slot_bits` スキップが
        T3b で有効化されるまでは概ね全 live スロット。有効化後は候補プールが
        縮小しポイントルックアップ数も減る）。これは `WHERE` 付き経路の
        「RLS 可視行数分」と同じオーダーであり、「フィルタなし経路だけが
        返却行数分で済む」という以前の見積りは撤回する（下記「コスト見積り」
        節に反映。per-entry 削減効果は走査・ヘッダデコード段と embedding
        デコード・コピー段に残る）
      - **却下した代替案: (b) ランキング後の権威検証＋不足分の再取得（refill）**:
        チャンク側複製値（非権威）でランキングした上位候補に対し返却直前に
        権威検証を行い、不一致・不可視で落ちた分は候補プールの次点（まだ
        ランキングに含めていない残りの live スロット）から補充して権威検証を
        繰り返し、k 件の可視行が揃うか候補プールを使い尽くすまで続ける案。
        候補段階のポイントルックアップコストを (a) より抑えられる可能性が
        あった一方、不可視行の embedding がいったんランキング計算に混入する
        点は変わらず、下記「不変条件」のうち「不可視行が順位・件数に影響
        しない」を構造的に満たさない（不可視行の距離値が「誰が k+1 番目の
        再取得候補になるか」に影響し、加えて再取得の有無・回数という観測可能な
        形で他テナントの存在情報が漏れうる）。この漏えい経路は本 ADR が
        維持すべき不変条件そのものと自己矛盾するため、コスト面の利点の有無に
        かかわらず**採用しない**（Rejected。codex-review P0 指摘への対応）
    - `SearchTimeFilter`・`batch_search` が metadata を用いる述語を渡す呼び出し
      経路を持つ場合は、KNN の `WHERE` 付き経路と同じ「RLS 直後・候補段階」で
      5 ステップ（4 点検証込み・metadata デコードあり）を実行する
    - **不変条件（T3a・T5 の検証対象に追加。Bugbot High 指摘への対応）**:
      1. **短い結果集合を返さない**: 可視行が k 件以上存在するにもかかわらず、
         不可視行の混入・除外に起因して k 未満の結果が返ることはない
         （採用した (a) では候補段階で既に可視行のみに絞られるため構造的に
         満たす）
      2. **不可視行が順位・件数に影響しない（存在情報漏えいなし）**: 他テナントの
         private 行を含む不可視行の embedding・存在が、返却される可視行の順位・
         件数・有無を左右しない（採用した (a) はランキング候補に不可視行を
         含めないため構造的に満たす）
    ...
    - **4 点検証の内容（fail-closed。codex-review P0 指摘・Bugbot High 指摘への
      ...
      （`WHERE` 系は O(RLS 可視行数)、フィルタなし系は採用した (a) 案により同じく
      O(候補になりうる全 live スロット数)〔`WHERE` 付き経路の「RLS 可視行数分」と
      同じオーダー〕。チャンク走査そのものを O(N) から変えない）、本 ADR が狙う
      per-entry コスト削減の
...
- **読み取り（フィルタなし経路。`WHERE`・宣言的フィルタ API を伴わない KNN・
  `SearchTimeFilter`・`batch_search`）（Bugbot High 指摘への対応で見積りを
  改訂。上記「権威検証を行うタイミング」節参照）**: チャンク走査・ヘッダデコード
  段の per-entry コスト（≈225ns/行の走査＋ヘッダデコード分）は `1/N` 償却が
  引き続き見込める（25,000 行・N=256 では約 98 エントリまで削減される計算）。
  一方、権威検証の母数は採用した (a) 案では、ランキング前の
  候補段階で全 live スロット（`slot_bits` スキップが T3b で有効化されるまでは
  概ね走査対象の可視候補全体）に対しポイントルックアップが発生するため、
  下記「metadata 必須経路」と同じ `O(RLS 可視行数)` 項を持つ（metadata の
  デコードは省略できる分、係数は metadata 必須経路より小さい）。「この経路の
  ポイントルックアップは最終的な返却行数分のみに限られる」という以前の見積り
  （ランキング後に絞ってから検証する設計を前提にしていた）は撤回する。
  ランキング後に不足分を再取得する案（(b)）は、ポイントルックアップの母数を
  抑えられる可能性があった一方、不可視行の embedding がランキング計算に混入し
  再取得の有無・回数を通じて他テナントの存在情報が漏れうるため却下した
  （上記「権威検証を行うタイミング」節「却下した代替案」参照。コスト面の
  利点は不変条件〔存在情報漏えいなし〕を満たさない代償としては採用しない）。
  KNN の arena 構築段（≈468ns/行 の残り ≈243ns/行 = f32
  デコード＋コピー）は連続コピー化により減少が見込まれる（定量は #362 の
  実測で確定させる。本 ADR では上限見積りとして提示するに留める）
- **読み取り（metadata 必須経路。`WHERE`・宣言的フィルタ API を伴うクエリ）**:
  ...
  別々に報告する。採用した (a) 案では、フィルタなし経路と metadata 必須経路の
  ポイントルックアップ母数のオーダーは近づくため、両者の差が縮小すること
  自体も T7 の実測項目に含める
