  ...
  - KNN・`SearchTimeFilter`・`batch_search` はチャンク走査で得たスロットの
    ...
    **権威検証を行うタイミングは「RLS predicate 通過後」であり、経路によって
    ...
    - KNN（`WHERE`・宣言的フィルタ API を伴わない場合）・`SearchTimeFilter`・
      ...
      次の 2 案のいずれかを主案として採用し、実装タスク分解時に選択する
      （いずれも下記「不変条件」を満たすことを実装タスクの検証対象とする）:
      - **(a) ランキング前の候補段階での権威検証（推奨・主案）**: チャンク走査で
        ...
        デコード・コピー段に残る）
      - **(b) ランキング後の権威検証＋不足分の再取得（refill）**: (a) のコスト
        増を避けたい代替案。チャンク側複製値（非権威）でランキングした上位候補に
        対し返却直前に権威検証を行い、不一致・不可視で落ちた分は「短い結果集合を
        黙って返す」のではなく、候補プールの次点（まだランキングに含めていない
        残りの live スロット）から補充して権威検証を繰り返し、k 件の可視行が
        揃うか候補プールを使い尽くすまで続ける。候補段階のポイントルックアップ
        コストを (a) より抑えられる可能性がある一方、不可視行の embedding が
        いったんランキング計算に混入する点は変わらないため、下記「不変条件」の
        うち「順位への不可視行の影響」を厳密には満たさない（不可視行の距離値が
        「誰が k+1 番目の再取得候補になるか」に影響しうる）。採用する場合は
        この残存差分を許容できるか（他テナントの存在情報が再取得の有無・回数
        という観測可能な形で漏れないか）を実装タスク分解時に別途検証する
      - 実装タスク分解時にいずれか一方を選び、選ばなかった案は Rejected として
        理由とともに記録する（本 ADR は選択の確定を行わない）
    ...
    - **不変条件（T3a・T5 の検証対象に追加。Bugbot High 指摘への対応）**:
      1. **短い結果集合を返さない**: 可視行が k 件以上存在するにもかかわらず、
         ...
         （(a) では候補段階で既に可視行のみに絞られるため構造的に満たす。
         (b) を採用する場合は refill ループが k 件充足または候補プール枯渇まで
         継続することを検証する）
      2. **不可視行が順位・件数に影響しない（存在情報漏えいなし）**: 他テナントの
         private 行を含む不可視行の embedding・存在が、返却される可視行の順位・
         件数・有無を左右しない（(a) はランキング候補に不可視行を含めないため
         構造的に満たす。(b) を採用する場合は上記の残存差分を明示的に評価し、
         許容できない場合は (a) へ切り替える）
    ...
    - **`slot_bits`／header 集計値によるスキップ最適化（T3b）は検査コストを
      ...
      使わない」原則の帰結）。(a)・(b) いずれの案でも、admission の可否は常に
...
- **読み取り（フィルタなし経路。`WHERE`・宣言的フィルタ API を伴わない KNN・
  ...
  一方、権威検証の母数は選ぶ案で異なる。**(a) を採用する場合**、ランキング前の
  ...
  **(b) を採用する場合**、ランキング後の上位候補＋refill 分のみに権威検証の
  母数を抑えられる可能性があるため「返却行数近傍」の見積りに近づくが、
  refill 発生率はコーパス中の不可視行の混在率に依存し、最悪ケースでは
  (a) に漸近する。KNN の arena 構築段（≈468ns/行 の残り ≈243ns/行 = f32
...
- **読み取り（metadata 必須経路。`WHERE`・宣言的フィルタ API を伴うクエリ）**:
  ...
  別々に報告する。(a) を採用した場合、フィルタなし経路と metadata 必須経路の
