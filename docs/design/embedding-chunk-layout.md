  ...
  - KNN・`SearchTimeFilter`・`batch_search` はチャンク走査で得たスロットの
    ...
    **権威検証を行うタイミング（確定方針。以下がこのトピックの唯一の定義であり、
    ...
    - **フォールバック契約（採用する再設計。本節の中核）**: chunked 経路の
      ...
      per-chunk フォールバックする**。フォールバック発動時は、当該 world 由来で
      すでにチャンク走査から収集していた候補（複製値検査を通過済みのものを
      含む）を**すべて破棄**し、`user_rows` の当該 `tenant_id` 範囲走査結果で
      **置換**する（両者を混在させない。フォールバックが「row-store 経路と同一の
      結果集合になる」という本節の主張は、この破棄・置換によって成立する）。
      他 world の候補・処理は一切影響を受けない。フォールバック先の row-store
      範囲走査は非再帰・一発限り（当該 world について chunked 経路へ再入しない）
      であり、その走査自体が `Err` を返す場合（`user_rows` 側 header デコード
      失敗等、row-store 経路が本来 `Err` にする破損クラス）は、その `Err` を
      そのまま呼び出し元へ伝播する（フォールバックが新たな情報を付加しない）。
      効果:
      1. **結果集合が row-store 経路と同一になる**ため、可視行の誤除外・短い
         結果集合の返却が構造的に起きない（Bugbot Medium 指摘への対応。ドリフトで
         live スロットが無関係な不可視行を指す場合も、その world 全体を権威
         range 走査へ切り替えるため、正しい占有者が検証されずスキップされる
         余地がない）
...
- **チャンク単位・スロット単位のいずれも、`slot_bits`／header 集計値による判定は
  ...
  - **除外判定自体の整合性検証（T8。上記の残存リスクへの対応。破損チャンクの
    ...
    経路には結線しない）。**T8 が実装されるまでは、`layout: chunked` テーブルの
