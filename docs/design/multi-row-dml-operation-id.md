...
- ステータス: Accepted（2026-09-23 オーナー承認。「判断記録」節参照。spec 側 RECOVER-11 の
  確定化は spec リポへ申し送り）
...
spec 側 RECOVER-11 の確定化は spec リポへ申し送りとし、確定時に本 ADR と差異が生じた
場合は spec を正として本 ADR・実装を追随させる。
...
| 条件 | spec 側 RECOVER-11 の確定化は spec リポへ申し送る。確定時に本 ADR と差異が生じた場合は spec を正として追随する |
| 判断日 | 2026-09-23 |
