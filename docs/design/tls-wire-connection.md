...
- **H3（ハンドシェイクの期限）**: SQL 表層と同じ `perform_server_handshake`
  ...
  で有界になる。
- **H4（TLS 下の同時接続超過）**: TLS を構成している場合、拒否経路は平文の
  503 応答を書かずに閉じる（`require` 下で平文バイト列を送出しないため、
  かつ TLS ハンドシェイクをしていない相手には意味のない応答になるため）。
  拒否ワーカーの中ではハンドシェイクしない（`RejectWorkerLimiter` の
  有界性を維持するため）。TLS 未構成時は既存の 503／`53300` 経路とバイト
  単位で同一のまま。
...
- TLS 構成時の同時接続超過を 503/`53300` で TLS 上に返すこと（H4）
