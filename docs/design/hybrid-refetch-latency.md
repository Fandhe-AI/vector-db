...
計測回数（`MeasurementConfig` の `measured_iterations`）は `NUM_QUERIES`（20）の
倍数（40）にしてある。倍数でないと round-robin で選ばれるクエリの一部だけが
1 回多く計測され、p95/median が特定クエリへ偏って重み付けされる
（codex-review P1 指摘・PR #325。`hybrid_latency_bench.rs::measure_stage` 参照）。
下表は倍数化後の再測定値であり、`queries`／`reached_visible_set` の分母は
`NUM_QUERIES`（20。再取得統計はクエリごとに 1 回だけ集計する別パスで求めるため
`measured_iterations` に依存しない。同ファイル参照）。

| 段 | corpus | provider 呼び出し回数（最大） | 観測 `fetch_k` 最大値 | p95（3 回の中央値） | median（3 回の中央値） |
| --- | --- | --- | --- | --- | --- |
| small（1,000 件） | no_refetch（連続値） | 1 | 400 | 211 µs | 198 µs |
| small（1,000 件） | tie_refetch（5 クラスタ） | 2 | 800 | 354 µs | 201 µs |
| large（20,000 件） | no_refetch（連続値） | 1 | 400 | 5,586 µs | 3,796 µs |
| large（20,000 件） | tie_refetch（5 クラスタ） | 5 | 6,400 | 12,533 µs | 10,565 µs |

`reached_visible_set`（再取得が可視集合サイズまで到達したクエリ数）はいずれの
段・モードでも 0/20 だった。今回の同点誘発コーパス（5 クラスタ）は
...
- 大規模段では、再取得ループが 1 回 → 5 回の provider 呼び出しへ伸びたことで
  p95 が約 2.2 倍（5,586 µs → 12,533 µs）、median が約 2.8 倍（3,796 µs →
  10,565 µs）に増加した。再取得ループの寄与は測定ノイズの範囲内ではなく、
  provider 呼び出し回数の増加に比例する形で明確に観測できる
- 小規模段でも provider 呼び出しが 1 回 → 2 回に伸びた影響で p95 が増加した
  （211 µs → 354 µs）が、median はほぼ変化していない（198 µs → 201 µs）。
