...
`ef=64`（既定値）に固定したまま HNSW から `oversample_k`（`>10`）件の
候補を取得し、各候補を**元の f32 ベクトル**で再採点してから上位 10 件を
選び直す評価（`recall_at_10_oversample_rescored`）を同じクラスタ構造
ありコーパス・同じクエリ集合で実測した:

| oversample_k | クラスタあり I8（f32 再採点後 Recall@10） |
| --- | --- |
| 10（再採点なしと同義） | 0.9050 |
| 20 | 1.0000 |
| 50 | 1.0000 |
...
**`ef=64` を変えずに候補数を 10→20 へ増やし f32 で再採点するだけで
Recall@10 は F32 と同水準（1.0000）まで完全に回復する。** これは
「I8 側の探索経路が F32 と異なる順序で収束し候補そのものを取りこぼす」
のではなく、**正解の大半（0.9050→1.0000 の差分）は `ef=64` の探索で
...
と結論づけるのは誤りだった（codex-review 指摘の是正）。
