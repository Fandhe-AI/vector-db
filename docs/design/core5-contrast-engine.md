...
> **本 ADR の位置づけ**: CORE-5 の公開境界（何を public 資産へ書けるか）と対照エンジンの
> 選定判断をベース側へ先に確定させるための記録。以下の設計判断はオーナー承認済み
> （2026-08-26）で、SSOT は `docs/spec/04-behavior/core-engine.md` CORE-5 の同日追記。
> 実装（`contrast_bench.rs`・`harness/contrast.rs`・`contrast-bench` feature・
> bench-contrast ジョブ）は PR #224 でマージされる。本 ADR が参照する新規ファイルは
> 本 PR 時点ではまだ存在しない。
