...
再測定によるギャップ解消はこの PR のスコア外とする。理由は次の通り: (1)
B0s は B4/B5 と**同一の交互実行**から得られる値のため、B0s だけを単独で
再測定しても既存の B4/B5 per-run 値・判断（「ノイズ帯と同程度」）との
対応関係が崩れる。整合させるには 4 条件 × 2 side × 5 ペアの全体（B4/B5/B0s
すべて）を再実行し、上表・per-run 表・判断節を丸ごと置き換える必要があり、
単一の P2 指摘（生データ記録漏れ）の範囲を超える。(2) 本リポの計測規約
