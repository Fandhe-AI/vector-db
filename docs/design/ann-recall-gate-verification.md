...
この変更は構築時のグラフ構造そのものは変えない契約（`repair_reachability_
inner` の `threads` の値によらずグラフはビット同一。上記 doc「Issue #449
追記」節「決定性の検証」参照）だが、Recall ゲート層 B・可視外非混入
テストが `#412`／`#515` 時点の実測値と変わらず通ることを本節で再確認する。
...
`repair_reachability` の削減（#447〜#449）はグラフ構造をビット同一に
保つ設計（同 doc「決定性の検証」参照）どおり、Recall ゲートに影響を
与えていないことを実測で確認した。全指標で `hnsw >= brute_force` かつ
`repair 削減後 == repair 削減前` の S8 決定規則を満たすため、閾値を
緩める必要はない。
