...
この結果、**wire 接続管理部分**のスレッド数は「接続スレッド ≤ 64」＋
「拒否ワーカー ≤ 16」＋「accept ループ本体 1」で合計 ≤ 81 本に有界化されて
いる。ただしこれはプロセス全体のスレッド数ではない——既定検索エンジン経路
では `engine::parallel_search` が検索 1 回あたり最大 `MAX_THREADS_PER_QUERY`
（＝16）本のワーカースレッドを追加生成しうる。この検索単位の上限とは別に、
複数クエリが同時実行された場合でもプロセス全体で共有する追加ワーカー予算の
上限として `MAX_TOTAL_EXTRA_WORKER_THREADS = 64` があり（後述の実測節参照）、
クエリ実行中のプロセス全体のスレッド数はこれを超えうる。
...
cargo test --release -p fandhe-vector-db-wire-server --test wire_concurrency_throughput --
...
- 受理経路 `std::thread::spawn`（生成失敗で panic → `recovery::fail_fast` に
  ...
  ログのみ）の非対称の是正。**wire 接続管理部分**のスレッド数が
  `MAX_CONNECTIONS` ＋ `MAX_REJECT_WORKERS` ＋ 1 で有界なため通常の ulimit
  下では到達不能であり（既定検索エンジン経路の `parallel_search` ワーカー
  を含めたプロセス全体では上限がこれより大きくなりうる点は上記のとおり）、
