...
観測されない。`udf_call` の p95 median 比のみ 1.378 と大きく振れているが、
絶対値は 754→1039µs（281µs の差）と udf_call 自体の絶対時間が最も短い
フェーズであり、他フェーズと異なり参照区間よりも小さい絶対差が相対比率を
大きく見せている（`benchmark-judgement-policy.md` §4 の「両ノイズ帯の比較式」の趣旨どおり、絶対時間
幅の小ささを踏まえると単発の外れ値 run（C-3 の 1099µs・C-2 の 1089µs）に
牽引された値であり、他 12 フェーズと同列の退行シグナルとしては扱わない）。
...
process3（A=5.37・C=5.45）が本セクション内で最も高い loadavg だが、B1・B2・
B5 いずれも process3 の値が min-of-5 に選ばれておらず、外れ値回が min-of-5
集計に混入してはいない（§4 冒頭の生データ参照）。
...
9590]（A pair5・C pair2 が外れ値）。参照区間 `vector_knn` の p95 実測ノイズ帯
は A=59.4%・C=57.1%（p50 のノイズ帯 A=19.7%・C=8.6% よりも大幅に広い。
p95 は分布の裾を見る統計量であり、共有 QEMU 環境ではこの規模の run-to-run
変動が生じる）。`hybrid_rrf` の p95 比（min 1.094・median 1.055）はこの
ノイズ帯（57〜59%）を大きく下回るため、悪化・改善いずれの根拠にもならない
（ノイズ帯内）。
