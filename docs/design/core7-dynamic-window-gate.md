...
- 複数試行（`CORE7_TRIALS`）を行う。試行内の統計量には劣化率（%）の**p95**を
  使い（`p95_degradation_pct`）、試行間はその p95 列の**中央値**を閾値と比較する
  （`median_degradation_pct`）。CORE-7 が定める量は単発クエリ経路の p95 劣化で
  あり、試行内統計量を中央値にすると「典型的な反復の劣化率」しか表さず、被検側
  （B）の遅い上位 5% だけが退行しても検出できない（codex-review 指摘。旧来の
  経路別独立 p95 差分比較より判別力が弱くなっていた）。試行内 p95・試行間
  中央値は別種のノイズに対する別処置であり、単一試行だけでは hosted runner
  上の突発的な計測スパイクが 1 回でも起きると誤 fail しうるため、試行間の
  中央値採用でその影響を緩和する（本コミットのローカル検証でも、複数試行中
  1 試行だけが大きく外れる事象を実際に観測しており、試行間中央値がその対策
  として機能することを確認した）。
...
- 判別力の希釈対策: A（対照）・B（被検）が共有する支配的なコスト（全走査）は、
  ...
  同一反復を指す）を使い、反復ごとに対で劣化率を取ってから**その試行内 p95**を
  取ることで共通コスト成分を相殺しつつ、CORE-7 が定義する量（p95 劣化）は
  維持する（ペア化差分によるノイズ低減。レビュー対応。詳細は下記「本ゲートの
  感度の限界」節）。
