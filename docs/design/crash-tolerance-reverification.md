...
| 3（部分 write-back、CI 分・固定シード 32 反復） | 常時 | 合格。32/32 反復で「正常に開けて行 1 の内容が完全一致」。オープン失敗（fail-closed 拒否）は 0 件（実測値。`cargo test -- --nocapture` で採取）。テストはこの `rejected == 0` を不変条件として明示的にアサートする（上述の構造上の制約により、これが期待される結果） |
...
- シナリオ 3 で「開けたが内容が最後のコミット時点と異なる」ケースは、CI 分・拡張分
  ...
  **この探索空間で必然的に得られる結果**である。テストはこの不変条件（`rejected == 0`
  かつ全反復オープン成功・内容一致）を明示的にアサートし、モデルの前提が崩れた場合に
  検知できるようにしている。
...
- **再検証で確認できた範囲**: 本シミュレーションモデルの下では、TASK-140/TASK-141 の
  永続化層は電源断シナリオ 1・2・4 において fail-closed（応答済みコミットの黙示的消失・
