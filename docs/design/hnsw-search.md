...
- 閾値候補 = 「全 rows 規模で `Improved` となる可視候補数の最大値」。ただし
  ...
  満たす点に限る
- 該当点が無ければ `DEFAULT_SPARSE_VISITED_MAX = 0` のまま現状維持
  （Rejected）——構造的事実（上記 1・3）により非破壊側の判断であることを
  計測前から明記する
...
- rows=10k（pooled 参照区間実測帯 24.03%）: 50%・25%・10% では
  `ratio(min)` が固定 ±5% 帯・実測帯のいずれも上回る一貫した悪化方向
  （1.05〜1.55x）。5%・2%（いずれも production 到達不能な informational
  参考値）では差が両ノイズ帯の範囲内で判定不能——それでも改善方向を示す
  点は無かった
- rows=100k（pooled 参照区間実測帯 59.02%。共有環境の負荷変動の影響が
  10k より大きい）: `ratio(min)` は 1.006〜1.125 で、10% と 2% はこの
  広い実測帯の内側にあり判定不能。50%・25%・5% は実測帯の範囲内〜境界
  だが、10 測定点すべてが改善方向でない点は共通して一貫している

事前登録した判定規則により、閾値候補（「全 rows 規模で `Improved` となる
可視候補数の最大値」）は**存在しない**。したがって `DEFAULT_SPARSE_VISITED_MAX
= 0`（常に dense。既存動作）を**現状維持（Rejected）**と判断する。構造的
事実（上記 1・3）どおり `VisitedBitmap::reset` の N 比例コストは今回の規模
（10k〜100k・可視候補数 216〜50,286）では `VisitedSparse` の `HashSet`
挿入コストを一貫して下回っており、閾値を 0 より大きくする根拠は本実測から
得られなかった。
...
- 共有 QEMU 環境の実測のため `benchmark-judgement-policy.md` §5 により本
  実測は「採用（Accepted）」の根拠にはできない。ただし本 Issue の結論は
  「現状維持（Rejected）」であり、構造的事実（可視候補数に対して
  `VisitedBitmap::reset` の N 比例コストが優位）とも整合するため、専有環境
  再実測は必須ではなく（現状維持側の判断は環境証拠力に依存しない）、
  結果が覆る可能性が高いと判断される場合にのみオーナー判断で実施する
