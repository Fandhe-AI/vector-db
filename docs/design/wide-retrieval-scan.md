...
### 例外: スカラー列 `ORDER BY`（Issue #915・SQL-25・TASK-209）

上表「順序」の**順序保証なし**は既定契約のままだが、`ORDER BY <スカラー列>
[ASC|DESC][, ...] LIMIT n` を明示的に付与した場合に限り決定的な順序を返す
（ベクトル順位付け形の `ORDER BY` とは構文上相互排他。混在は `42601`）。構文・
比較規約・実行経路の詳細は `docs/design/scalar-order-by-scan.md` を参照。
