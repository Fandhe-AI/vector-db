...
- 同点（スコアが等しい）要素のタイブレークは常に **id 昇順**とする。
  ...
  `(tenant_id, id)` 昇順へ拡張される。ただし**実装上の基準は選出段の候補識別子
  （常駐行列の行スロット番号）昇順**であり、`ResidentMatrix::build` は渡された行を
  並べ替えないため、両者が一致するのは「行を `(tenant_id, id)` キー順で常駐行列へ
  渡す」という事前条件を呼び出し元が満たす場合に限る（`Storage` の行キー順で
  スキャンした結果をそのまま渡せば自然に満たされる。`ResidentMatrix::build` の
  ドキュメンテーションコメント「事前条件: 行の並び順」参照）。事前条件を満たさない
  並びでも結果はスロット昇順で決定的であり、非決定性は生じない。単一テナント内では
  従来どおり id 昇順に退化し、CPU 経路・GPU 経路の間で順序が食い違わないことは
...
- `LIMIT`/`k` による切り詰め（`truncate`）は、切り詰め前の順序が上記契約を満たして
  ...
  非決定的になりうる点に注意する。
- **同点グループの RRF 順位は平均順位とし、候補 id の並びに依存しない**
  （Issue #307。対象ビヘイビア: SEARCH-1。経緯・分析は
  `docs/design/hybrid-recall-regression.md`「小規模段ゲート未達の engine 側原因
  調査（Issue #307）」節参照）。`hybrid.rs::accumulate_ranked` は同点グループ
  （スコアが `total_cmp` で等しい連続区間）に区間の平均 1-based 順位を割り当てて
  `weight / (k_const + rank)` を加算する。この不変条件は「同点グループが
  `cfg.pool_depth()` 内に収まる場合」に限って成立する（同点グループが
  `pool_depth` の境界を跨いで分断される場合、`pool_depth` に含まれない側の要素は
  そもそも融合対象に入らないため、id の並びが「どの要素が `pool_depth` 内に
  残るか」に影響しうる。この境界跨ぎの扱い自体は本項の対象外——上記「`LIMIT`/`k`
  による切り詰め」と同種の既知の制約であり、`is_sorted_desc_id_asc` が検証する
  入力契約〔スコア降順・同点は id 昇順〕の範囲でのみ決定的である）。本項が保証する
  のは、同点グループ全体が `pool_depth` 内に収まっている場合に、そのグループ内の
  各候補が得る融合寄与が候補 id の並びに依存しないことである。
