...
PoC-10 が発見した「RRF 融合スコアが同点のとき、`HashMap` 由来の走査順序と
`sort_unstable_by`（同点タイブレークを保証しない不安定ソート）の組み合わせにより、
同一クエリでも Top-k の並びが実行ごとに変わりうる」という非決定性が、本リポの
`engine` クレートへ持ち込まれていないかを判定し、判定結果を記録する。修正が必要な
場合は修正し、不要な場合は根拠とともに現状維持の判断を残す（TASK-84 の成果物は
...
**修正不要・現状維持**。`engine` 本体の RRF 融合経路は、PoC-10 が指摘した「`HashMap`
由来の走査順序」「同点タイブレーク欠如」のいずれの原因も持たない。
