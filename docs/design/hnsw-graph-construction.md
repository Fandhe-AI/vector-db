...
- **`repair_reachability`（全ノード挿入後の後始末パス）**: 上記の残差
  ...
  リスト方式にした。反復回数の上限（層のメンバ数の定数倍。理論上の収束
  保証はないため fail-closed に打ち切る）までは次数上限を維持する
  `shrink_links` 経由で厳密に修復し、収束せず上限に達した残りは `connect`
  のみ（次数上限超過を許容）で強制的に閉じる。安全側 = 全ノードの到達性を
  最終的に必ず保証する（次数上限は通常の挿入経路・大半の修復では従来どおり
  保証されるが、この残差フォールバックのみ例外的に超過し得る）。
...
### 実測結果（本開発環境・単発計測。#423 是正前の基線）

以下は「逆方向リンクの到達性保証」節・「`search_layer` の停止・受理判定」
節の是正（PR #423。`HnswIndex::build` 末尾へ `repair_reachability` を追加）
より**前**の計測値である。この修復パスは通常経路で BFS を 1 回追加する
ため、実際の構築時間は本節の値から変化している。再測定は `make
bench-hnsw-build`（Actions 非配線の手動専用ベンチ）を承認済み計測環境で
実行して更新することとし、本 PR の時点では申し送りとする（本節の伸び方の
分類自体はスケーリングの傾向を示す情報提供専用であり、spec 由来の合否
閾値ではないため、値の陳腐化は CI 判定に影響しない）。
