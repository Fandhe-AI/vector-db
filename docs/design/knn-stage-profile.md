...
| S5'→S5_scalar | 0.8 | Top-k 選出（`BinaryHeap` 部分ソート）分（単線・逐次走査条件下） |
...
4. **#366（距離計算/Top-k の 2 段分離）**: Top-k 選出コストの分離には
   ...
   並列化コストを含まず Top-k 選出のみへ帰属できる。距離計算自体
   （7.8 ns/行）が S5_scalar の大部分を占め、Top-k 選出は相対的にさらに小さい。
