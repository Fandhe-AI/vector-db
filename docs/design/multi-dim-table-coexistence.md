...
TASK-91（対象ビヘイビア: TABLE-2）に関連し、これまで単一次元のみの実測に
留まり、複数次元テーブルを実際に共存させた実測が無かった。本 ADR はその
実測・実装検証の結果と判断材料を記録する。TABLE-2 自体の契約内容は
`docs/spec/04-behavior/data-model.md` を参照（本リポからは転記しない）。
...
検証を現行アーキテクチャの 2 層に分けて行った。

### カタログ層

384 / 768 / 1536 次元（一般的な埋め込みモデルで使われる代表値）を宣言した
3 テーブルを単一 `Storage`（同一 redb ファイル）に `create_table` し、
...
- **正しさ（検証範囲を限定して確認）**: カタログ層（3 テーブルの次元宣言・
  ...
  存在は本検証の範囲外であり、`validate_embedding_dim` はテストから直接
...
より広い正しさの確認は、行ストアのテーブル関連付け本実装（後続タスク）後に
別途行う必要がある。
