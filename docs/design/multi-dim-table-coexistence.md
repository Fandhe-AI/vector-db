...
- **`scan_page` 全走査時間は行数の増加に対しやや超線形**（mixed は baseline
  の 3 倍の行数に対し約 5.8 倍の所要時間）。絶対値は両条件とも数 ms オーダー
  であり実運用上の懸念は小さいと考えるが、比例から乖離しており、次元混在
  自体の影響かは今回の実測だけでは切り分けられないため、今後の実データでの
  再実測時に注視する。
