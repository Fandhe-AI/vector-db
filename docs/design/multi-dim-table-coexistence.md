...
| ------ | -------- | --- | --- | --- | ----- | -------- |
| baseline(single 768) | 40 | 5.587ms | 11.318ms | 11.974ms | 239.426ms | 3341.3 |
| mixed(384/768/1536) | 120 | 5.211ms | 6.316ms | 14.148ms | 673.945ms | 3561.1 |

| config | scan_page 全走査 rows | scan_page 全走査 elapsed | DB ファイルサイズ |
| ------ | --------------------- | ------------------------ | ------------------ |
| baseline(single 768) | 800 | 1.075ms | 5,738,496 bytes |
| mixed(384/768/1536) | 2400 | 2.351ms | 19,664,896 bytes |
...
- **書き込みスループット（rows/sec）は単一次元と同水準**（baseline 約 3341
  rows/sec、mixed 約 3561 rows/sec）。3 テーブルをバッチ単位で巡回
...
- **`scan_page` 全走査時間は行数の増加とおおむね比例**（mixed は baseline
  の 3 倍の行数に対し約 2.2 倍の所要時間）。絶対値は両条件とも数 ms オーダー
  であり実運用上の懸念は小さい。
...
テーブルのカタログ共存（宣言・永続化・境界分離）と、可変長行データの単一
行ストアへの物理的な混在保存（ラウンドトリップの正しさ）については、性能面の
劣化なく support する**と判断する（Proposed）。テーブル別の行絞り込み・書き込み
時の次元 fail-closed 検証を含む「TABLE-2 の完全な正しさ」の確認は、行ストアの
テーブル関連付け本実装（後続タスク）後に別途行う必要がある。
