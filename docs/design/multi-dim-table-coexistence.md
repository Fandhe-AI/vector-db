...
| ------ | -------- | --- | --- | --- | ----- | -------- |
| baseline(single 768) | 40 | 5.795ms | 11.452ms | 11.762ms | 245.219ms | 3262.4 |
| mixed(384/768/1536) | 120 | 5.905ms | 9.085ms | 14.263ms | 728.419ms | 3294.8 |

| config | scan_page 全走査 rows | scan_page 全走査 elapsed | DB ファイルサイズ |
| ------ | --------------------- | ------------------------ | ------------------ |
| baseline(single 768) | 800 | 3.114ms | 5,738,496 bytes |
| mixed(384/768/1536) | 2400 | 3.991ms | 19,009,536 bytes |
...
- **書き込みスループット（rows/sec）は単一次元と同水準**（baseline 約 3262
  rows/sec、mixed 約 3295 rows/sec）。異なる次元のテーブルへ交互に書き込む
...
- **`scan_page` 全走査時間は行数にほぼ比例**（mixed は baseline の 3 倍の行数
  で約 1.28 倍の所要時間）。異なる次元の行が同一テーブル（`ROWS_TABLE`）内に
  混在していることによる読み出し側の異常な劣化は見られなかった。
