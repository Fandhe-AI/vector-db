...
- **層 B**（`crates/wire-server/tests/three_client_e2e.rs`）: 実 `psql`・
  ...
  保証するため）。C1〜C4 それぞれを 3 クライアント全てで実行し、各クエリ
  固有の構文・列構成・型変換が独立オラクルと一致することを検証する
  （C2 は `id, lang` の 2 列を返して Text 型変換を検証し、C4 は全行の
  `body` に含まれない語をクエリ語に使って密のみのランキングへ縮退させ
  C1 と同じオラクルで照合する。codex-review P2 指摘・PR #210）。
