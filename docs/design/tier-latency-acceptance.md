...
実測値そのものは未取得のため、常駐 Ollama を用意した環境での再実行時に記録する
テンプレートとして扱う。
...
実測後、標準出力に記録された実測値・pass/fail・routing 結果を確認し、すべて合格で
あれば本ドキュメントのステータスを Accepted に更新し、PLAN-4/6/7 を「充足」へ
書き換える。`.github/workflows/bench.yml` に `bench-tier` ジョブは存在しない（GitHub
ホステッド runner に常駐 Ollama が無く、self-hosted は組織承認済み例外の範囲外の
ため。PR #269 Codex 指摘）。
