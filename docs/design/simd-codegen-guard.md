...
3. 各カーネルのシンボルを mangled 名（legacy `_ZN...17h<16進>E:` と
   ...
   ラベル行 〜 対象シンボルを参照する `.size` ディレクティブ（LLVM が各関数の
   直後に `.Lfunc_end<N>:` に続けて出力する `.size <symbol>, .Lfunc_end<N>-
   <symbol>`）を関数本体の境界として抽出する。単純に「次に現れる
   `.Lfunc_end[0-9]+:` まで」を境界にすると、対象関数自身の `.Lfunc_end` が
   欠落していた場合に後続関数の `.Lfunc_end` を取り違えて後続関数の命令列
   まで誤カウントする fail-open を招くため、対象シンボルの完全一致を伴う
   `.size` ディレクティブを境界確定の拠り所にしている（codex-review P1
   指摘）。
...
6. シンボルが 0 件・2 件以上（曖昧）・関数本体抽出失敗（対象シンボルの
   `.size` ディレクティブ不在）はいずれも ERROR（fail-closed）。
...
- `#[target_feature]` の付いた private 関数は release ビルドでデッドコード
  ...
  ことで回避した。
- 関数本体の境界（対象シンボルの `.size` ディレクティブ）探索を
  `tail | awk '/marker/{exit}'` のようなパイプ経由にすると、マッチ行で
  awk 側が早期に `exit` した際、まだ書き込み中の上流 `tail` が閉じられた
  パイプへ書き込もうとして `SIGPIPE`（終了コード 141）を受け取る。
  `set -euo pipefail` の下ではこれがそのままスクリプト全体の異常終了に
  なる（`.s` ファイルが数十万行あるため必ず再現する）。awk 単体に対象
  ファイルを直接読ませる方式（パイプの上流プロセスを持たない）に変更し、
  マッチ行で `exit` して早期終了しても SIGPIPE が構造的に発生しないように
  して回避した。
