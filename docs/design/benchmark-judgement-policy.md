...
2. **実測帯**: 同一計測セッションで得た「変更を含まない参照区間」の run-to-run
   幅。**固定相対帯と同じ単位系（`before` を分母とした相対比率）で算出し、
   絶対時間幅では比較しない**（下記「両ノイズ帯の比較式」参照）。

**両ノイズ帯の比較式（単位・正規化の統一）**

固定相対帯・実測帯はいずれも「対象区間の `before` 値を分母とした相対比率」で
算出し、**同じ単位（無次元の比率）で比較する**。絶対時間幅（µs 等）どうしの
比較や、相対比率と絶対時間幅を混在させた比較は行わない。
...
- 実測帯を超えるとは: `|ratio - 1.0| > reference_band / 2` を指す
  （`reference_band` は区間の全幅なので、`1.0` を中心とした片側許容幅は
  半分の `reference_band / 2` とする）。
...
（`ratio = 0.9`、固定相対帯 ±5% を超えるため `Regressed` 側に判定候補）、
参照区間が同一セッションで 1000〜1050 µs だった場合、`reference_min = 1000`
として `reference_band = (1050 - 1000) / 1000 = 5.0%`。実測帯の判定は
`|0.9 - 1.0| = 0.10 > 0.05 / 2 = 0.025` となり実測帯も超過するため、
