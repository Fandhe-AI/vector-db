...
1. **固定相対帯**: ±5%（`benches/harness/dot_kernel.rs::classify_change` の
   ...
   `Neutral` / `Regressed` に分類する。単位は比率（無次元）であり、
   絶対時間幅は用いない。
2. **実測帯**: 同一計測セッションで得た「変更を含まない参照区間」の run-to-run
   幅。**固定相対帯と同じ単位系（無次元の相対比率）で算出し、
   絶対時間幅では比較しない**（分母は参照区間自身の `reference_min`。
   詳細・式は下記「両ノイズ帯の比較式」参照）。

**両ノイズ帯の比較式（単位・正規化の統一）**

固定相対帯・実測帯はいずれも「自身の区間の実測値を分母とした相対比率」で
算出し、**同じ単位（無次元の比率）で比較する**。絶対時間幅（µs 等）どうしの
比較や、相対比率と絶対時間幅を混在させた比較は行わない（対象区間側は
`before` を分母、参照区間側は `reference_min` を分母とし、両者とも
「無次元の比率」という単位でのみ揃える。分母となる実測値そのものは区間ごとに
異なってよい）。

- 対象区間の比率: `ratio = after / before`（§3 で得た min-of-N または median）。
  固定相対帯の判定は `classify_change(ratio, 0.05)` のとおり。
- 参照区間の実測帯幅（相対）:
  `reference_band = (reference_max - reference_min) / reference_min`
  （同一計測セッションで得た参照区間の run-to-run 値列から算出。`reference_min`
  を分母とする——固定相対帯の `noise_band` が `1.0`（＝before）を基準とする
  比率であることに合わせ、参照区間側も自身の最小値を基準の分母とする）。
- 実測帯を超えるとは: `|ratio - 1.0| > reference_band` を指す
  （`reference_band` は参照区間の run-to-run 全幅であり、これを片側許容幅として
  そのまま使う。半分に縮めない——`reference_band` は「ノイズだけでも動きうる
  全幅」の実測であり、対象区間の before/after 差をその半分未満に制限する
  根拠はない。`docs/design/redb-insert-reserve-zero-copy.md`「決定規則の適用」
  節の先例も「差が同一実行群の参照区間の run-to-run 幅を上回ること」を採用
  条件としており、全幅を基準とする）。
- 記録する参照区間の値は、算出済みの相対比率（%）を主とし、算出元の絶対値
  （run ごとの µs 等）は再現性のため併記する（例: 「参照区間 X: 1000〜1050 µs
  → reference_band = 5.0%」のように、絶対値と相対値の両方を明記する）。

**例（単位混在による判定の逆転を防ぐ）**: 対象区間が 100→90 µs
（`ratio = 0.9`。レイテンシ系は `ratio < 1.0` が短縮＝改善方向であり、
`classify_change(0.9, 0.05)` は `ratio <= 1.0 - 0.05` を満たすため `Improved`
と判定候補になる）、参照区間が同一セッションで 1000〜1050 µs だった場合、
`reference_min = 1000` として `reference_band = (1050 - 1000) / 1000 = 5.0%`。
実測帯の判定は `|0.9 - 1.0| = 0.10 > 0.05` となり実測帯も超過するため、
両ノイズ帯を超える有効な変化として扱う（絶対時間幅どうしで比較し
「対象の絶対差 10 µs < 参照区間の絶対幅 50 µs だからノイズ帯内」と
誤読しないこと。単位を相対比率に統一すればこの逆転は起きない）。

非退行判定（参照区間方式）は Issue #401 §2.2 の閾値を既定として継承する。
p50 が before の ×1.05 以下、p95 が before の ×1.10 以下であれば非退行とし、
超過した場合は上記の実測帯算出式で before 側の run-to-run 幅（相対比率）と
比較してノイズ帯内か退行の疑いかを切り分ける。
...
| 絶対閾値ゲートの確定判定（TASK-83 条件7・CORE-6/16） | **不可** | 可 | 不可（GPU 非搭載・専有性なし） |
| CORE-7（動的窓劣化率ゲート。単発クエリ p95 の相対劣化率・試行間 median） | 可（`bench-batch` は絶対値ではなく同一プロセス内 A/B の相対劣化率で判定するため、共有 QEMU 環境でも判定可能。`docs/design/core7-dynamic-window-gate.md`） | 可 | **可**（既定のアクティブゲート。`.github/workflows/bench.yml` の `bench-batch` job が `ubuntu-latest` で常時実行） |
