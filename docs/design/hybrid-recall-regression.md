...
`accumulate_ranked`（`crates/engine/src/hybrid.rs`）を同点グループ（スコアが
`f64::total_cmp` で等しい連続区間）へ区間の平均 1-based 順位を与える方式へ変更した
（実装の詳細・不変条件は同関数のドキュメンテーションコメント参照）。これに伴い
`soft_boost_confirm_cap` の保証下限導出（`crates/engine/src/hybrid.rs`）も
同点グループの平均順位を前提にした形へ更新済み。

この改善は受け入れ条件 3（engine 側の改善余地があれば実装する）に基づく実装であり、
SEARCH-1／SEARCH-3 に対応づく。RRF の同点時の順位規約（平均順位）自体は
`RrfConfig::default()` が表す等重み・pool 200・k 60 の spec 採用構成には含まれない
実装判断であり、この規約自体の spec 側確定はオーナー判断待ちのまま残る。

診断のため既定値の掃引・代替方式（競争順位方式・平均逆数方式等）の比較も行ったが、
既定値変更はオーナー判断事項のため本 Issue では採用せず、平均順位方式を採用した。

改善後も到達不能な正解（構造的に密・疎いずれの観測にも手がかりが残らないケース）が
残っており、「疎・密チャネルの lossy view」（本ドキュメント既存節）が課す構造的な
難易度に起因する。層 B が依然 red の場合はコーパス生成条件の spec 側確定が必要になる
可能性があり、受け入れ条件 4 に基づきオーナーへ報告する。
