...
**採用した契約**: 非正スコア（`score <= 0.0`）の候補は RRF 融合のランク付け対象
から外す（`hybrid.rs::trim_non_positive_score_tail`）。密（`CandidateHit::score`）・
疎（`ScoredDoc::score`。BM25 は語一致なしで 0）いずれも、非正スコアは「クエリとの
シグナルが一切ない」ことを意味し、順位付けする意味がないとみなす。拡張取得列は
呼び出し元契約によりスコア降順のため、非正スコアの範囲は必ず末尾の連続区間になり、
先頭から見て最初に非正スコアが現れた位置で単純に切り詰めれば足りる。切り詰めに
先立つ契約検証（有限性・ソート順・重複 id。`validate_extended_pool`）は切り詰め
**前**の拡張取得列全体に対して行い、この検証範囲は変更していない。

この切り詰めにより、密・疎いずれも「取得済み範囲の末尾に非正スコアが 1 件でも
含まれていれば、その時点で正スコアの候補は取得済み範囲に全件含まれている」
（拡張取得列はスコア降順のため、未取得の候補は必ず非正スコア）ことを利用し、
再取得の判断（`exhaustive`）へこの事実を反映した。結果として、無関係文書の同点尾
そのものが境界完全化の対象外になり、再取得ループが可視集合全体まで伸びる経路を
断った。
...
**hits20 への影響**: 小規模段は 182 → 180（下記「実測結果」節）。非正スコアの
候補が偶然正解集合に含まれていたケース（無シグナルにもかかわらず疎密いずれかの
1 チャネルで rank 上は残っていた分）が、非正スコアの順位付け除外により結果から
外れたことによる。大規模段は 385/648 のまま変化なし（本フィクスチャでは大規模段の
hits20/hits100 算出対象クエリに非正スコア候補由来の正解ヒットが含まれていな
かった）。
...
融合（180/202）はいずれの単体チャネルも下回らない。上表の小規模段 `hits20` は
下記「Issue #320 大規模段追加調査: 非正スコア候補の順位付け除外」節の変更適用後
の値（適用前は 182）。大規模段は同節の変更適用前後で数値の変化なし（385/648 のまま）。
