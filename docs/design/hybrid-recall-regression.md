...
グループ全体を含める（境界が取得済み範囲内で確定できない場合は、観測できた範囲を
保持したまま位置ベースの `pool_depth` 件切り詰めへフォールバックする。Issue #320
codex-review P1 指摘対応。当初は決定的・id 非依存にそのグループを丸ごと除外して
いたが、全件同点コーパスで密・疎両チャネルが空になり検索結果自体が空になる機能
...
| 大規模 | 20,000 | 100 | 997 | 421 | 381 | 707 | 651 | 0.9050 | 0.9208 |
...
戻した。Issue #310（`GroupEnd`＋境界同点グループ完全化。丸ごと除外フォールバック
時点）の適用前、`hits20` は小規模段 171・大規模段 328、`hits100` は大規模段 645
であった（`ceil20`／`ceil100`／`total_correct` はいずれも上表と同一。フィクスチャ
非変更のため不変）。両段とも `hits20`（大規模段は `hits100` も）の到達率が改善
している。さらに Issue #320（境界同点グループを完全化できない場合のフォールバック
を「丸ごと除外」から「観測範囲を保持し位置ベースで `pool_depth` 件へ切り詰め」へ
変更。上記「Issue #310: engine 側改善」節参照）適用後は、大規模段の `hits20` が
385→381・`hits100` が 653→651 とわずかに減少した（小規模段は本コーパス規模では
境界が完全化できないケースが実測上発生せず不変）。これは安全側フォールバックの
選び方の違いによる実測差であり、`ceil20`／`ceil100`／`total_correct` はいずれも
不変のため測定前提自体は崩れていない。数値そのものの固定値ゲートに加え、spec 由来
の絶対閾値に対する pass/fail 判定は既存どおり層 B（`environment: recall-gate` の
secrets 経由）側で扱う。
