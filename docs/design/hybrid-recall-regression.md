...
密チャネル（one-hot 内積）・疎チャネル（BM25）いずれもスコアが離散値に偏るため、
`pool_depth` 内に大きな同点グループが生じる。`hybrid.rs::accumulate_ranked` は
同点グループ内の各要素にも列に現れる位置（同点内では候補 id 昇順）をそのまま
個別の順位として割り当てる仕様であり、同点グループ内での融合寄与が「候補 id の
並び」という、RRF が本来意味を持たせたい「意味的な近さ」とは無関係な鍵に依存する。
この依存が小規模段の Recall@20 劣化の主要因であることを診断的に特定した。

### 診断的に検証した代替方式（production へは未採用）

同点グループへ区間の平均 1-based 順位を割り当てる方式（融合寄与が候補 id の並びに
依存しなくなる）を含む複数の代替方式（競争順位方式・平均逆数方式等）を診断・実験
コードとして掃引・比較し、小規模段の Recall@20 改善余地があることを確認した
（受け入れ条件 3）。

ただし RRF の同点時の順位規約（平均順位）自体は `RrfConfig::default()` が表す
等重み・pool 200・k 60 の spec 採用構成には含まれない実装判断であり、この規約自体の
spec 側確定はオーナー判断待ちのため、**production の融合挙動
（`hybrid.rs::accumulate_ranked`・`rrf_fuse`）は変更していない**。既定値
（`k_const`・`pool_depth`・重み・BM25 パラメータ）の変更も同様にオーナー判断事項の
ため本 Issue では採用しない。

### 到達不能な正解（fixture の構造的難易度）

改善余地を除いても到達不能な正解（構造的に密・疎いずれの観測にも手がかりが残らない
ケース）が残っており、「疎・密チャネルの lossy view」（本ドキュメント既存節）が課す
構造的な難易度に起因する。層 B が依然 red の場合はコーパス生成条件の spec 側確定が
必要になる可能性があり、受け入れ条件 4 に基づきオーナーへ報告する。

### オーナー報告事項

- RRF の同点時の順位規約（平均順位）は、小規模段 Recall@20 の改善余地として診断的に
  確認済みだが、spec 採用構成（等重み・pool 200・k 60）の範囲外の実装判断であり
  production 未採用のまま。spec 側で確定する場合は対応する定義変更と対にして
  実装する
- 改善余地を除いても到達不能な正解は fixture の lossy view 由来であり、層 B が
  依然 red の場合はコーパス生成条件の spec 側確定が必要になる可能性がある
...
| 大規模 | 20,000 | 100 | 997 | 421 | 328 | 707 | 645 | 0.7791 | 0.9124 |
